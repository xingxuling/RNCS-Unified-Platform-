...
if(!failures.length){
  ...
  if(!gates.entries.every(entry=>entry.svg?.sha256&&entry.vector_png?.sha256&&entry.phase6_5_baseline?.sha256&&entry.drawing_mesh?.sha256&&entry.drawing_mesh_root&&entry.deformed_mesh_root&&entry.final_drawing_ir_root&&entry.full_body_drawing_certificate_root))failures.push('STATIC_AB_OR_MESH_EVIDENCE_INCOMPLETE');
  ...
  if(frames.drawing_backend!=='fullbody-bezier+quadratic-cage'||frames.deformation_backend!=='rncs.quadratic-3x3-drawing-cage.v0.1'||frames.raster_backend!=='librsvg')failures.push('VECTOR_DEFORMATION_BACKEND_CONTRACT_INVALID');
  ...
  if(!frames.frames.every(frame=>frame.lower_body_status==='represented-in-canonical-morphology-v0.2-candidate'&&frame.full_body_drawing_certificate_root&&frame.drawing_mesh_root&&frame.deformed_mesh_root&&frame.deformation_controls_root&&frame.final_drawing_ir_root&&frame.file?.sha256))failures.push('FRAME_ROOT_CHAIN_INCOMPLETE');
  ...
  if(backend.supersample_factor!==2||backend.external_visual_model!==false||backend.drawing_mesh_ir!=='rncs.anime-drawing-mesh-ir.v0.1'||backend.deformation_backend!=='rncs.quadratic-3x3-drawing-cage.v0.1')failures.push('BACKEND_RECEIPT_INVALID');
...
console.log(JSON.stringify({format:'rncs.phase6-6-verification.v0.2',passed:failures.length===0,failures,evidence_dir:dir},null,2));
