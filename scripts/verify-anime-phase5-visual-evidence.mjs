...
const first=build(),second=build();for(const key of stableKeys)assert.equal(second[key],first[key],`Non-deterministic stable evidence root: ${key}`);assert.equal(second.status,'blocked');assert.equal(second.provider_code,'MODEL_MISSING');assert.equal(second.media.mp4,null);assert.equal(second.media.wav,null);process.stdout.write(`${JSON.stringify({ok:true,stable_roots:stableKeys,roots:Object.fromEntries(stableKeys.map(key=>[key,second[key]])),volatile_file_sha256:true,media:{mp4:null,wav:null}},null,2)}\n`);
