...
- Added Android Emulator/WebView evidence for the same explicit audio/VSR seam: a Gradle 9.5.1 / SDK 35 debug APK retains the embedded plan, VSR receipt root and Web Audio scheduling receipt; native audio, physical-device and human-listening claims remain closed.
- Reused the existing VSR browser CacheStorage provider for bound audio target bytes; clean Chromium and API 35 Android WebView candidates both recorded a first-load miss/write followed by a cold-reload hit with the same revision/manifest and VSR receipt roots. No new audio cache schema, quota guarantee, continuous-stream, native-audio or human-listening claim was added. Evidence: `evidence/BUILD_AUDIO_VSR_CACHE_CANDIDATE_v0.1.json`, `evidence/ANDROID_AUDIO_VSR_CACHE_CANDIDATE_v0.1.json`.
