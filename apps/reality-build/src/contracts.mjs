...
function normalizeAssetCache(input){
  ...
  const persistAccesses=raw.persist_accesses??raw.persistAccesses??false;
  return{enabled:raw.enabled!==false,max_bytes:maxBytes,persist_accesses:persistAccesses};
}
...
export function validateBuildRequest(r){
  ...
  if(r?.asset_cache!==undefined){need(typeof r.asset_cache.enabled==='boolean','ASSET_CACHE_ENABLED_INVALID','asset_cache.enabled');need(r.asset_cache.max_bytes===null||Number.isSafeInteger(r.asset_cache.max_bytes)&&r.asset_cache.max_bytes>=0,'ASSET_CACHE_MAX_BYTES_INVALID','asset_cache.max_bytes');need(typeof r.asset_cache.persist_accesses==='boolean','ASSET_CACHE_PERSIST_ACCESSES_INVALID','asset_cache.persist_accesses');}
