...
import {buildBrowserBehaviorRuntime,buildBrowserVSRRuntime,buildGameSource,renderIndexHtml} from './runtime-template.mjs';
...
export function buildWebRelease({project,request,identity,bakeDir,outRoot,assetManifest,publicAssetManifest,runtimeEvidence,assetDatabase}){
  ...
  writeText(path.join(dir,'runtime.js'),buildBrowserBehaviorRuntime());writeText(path.join(dir,'vsr-runtime.js'),buildBrowserVSRRuntime());writeText(path.join(dir,'game.js'),buildGameSource());writeText(path.join(dir,'build-data.js'),buildDataSource(payload));
