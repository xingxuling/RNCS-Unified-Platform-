...
import {materializeSpatialAssetPayloads,writeSpatialAssetPayloadManifest} from './presentation-assets.mjs';
import {buildBrowserBehaviorRuntime,buildBrowserRSRRuntime,buildBrowserSpatial3DRuntime,buildBrowserVSRRuntime,buildBrowserUIInputRuntime,buildGameSource,renderIndexHtml} from './runtime-template.mjs';
import {createBuildUIInputRuntime} from './ui-input.mjs';
...
import {compileAudioTargetPlan} from '@taowind/audio-target-runtime';
...
function runtimePayload(project,assets,request,identity,target,runtimeEvidence,assetDatabase=null,publicAssetManifest=null,presentationAssets=null){return{project:artifactProject(project),assets,asset_database:assetDatabasePayload(assetDatabase,publicAssetManifest),ui_input_runtime:createBuildUIInputRuntime(project),build:{format:'reality-build.runtime.v0.2',build_id:identity.build_id,build_key:identity.build_key,target,mode:request.mode,quality_profile:request.quality_profile,app:request.app,build_time:request.build_time},runtime_evidence:runtimeEvidenceSummary(runtimeEvidence),spatial_runtime:{state_root:runtimeEvidence?.spatial_snapshot?.stateRoot??null,causal_delta_root:runtimeEvidence?.spatial_causal_delta?.deltaRoot??null,manifest_root:runtimeEvidence?.spatial_runtime_manifest?.manifest_root??null,navigation_manifest_root:runtimeEvidence?.navigation_manifest?.manifest_root??null,manifest:runtimeEvidence?.spatial_runtime_manifest??null,snapshot:runtimeEvidence?.spatial_snapshot??null,navigation_manifest:runtimeEvidence?.navigation_manifest??null,world:runtimeEvidence?.spatial_world??null},spatial_3d_runtime:{preferred_backend:'webgpu',fallback_backend:'canvas2d-projective',initial_state_root:runtimeEvidence?.spatial_initial_snapshot?.stateRoot??null,world:runtimeEvidence?.spatial_world??null,snapshot:runtimeEvidence?.spatial_initial_snapshot??null,scene:runtimeEvidence?.spatial_scene??null,frame_plan:runtimeEvidence?.spatial_frame_plan??null,frame_root:runtimeEvidence?.spatial_frame_plan?.frameRoot??null,animation_policy:request.presentation_candidate?.presentation?.animation_policy??project?.spatial3d?.presentation_animation_policy??null,sequence_frame_projection:request.presentation_candidate?.presentation?.sequence_frame_projection??project?.spatial3d?.sequence_frame_projection??null,world_body_event_delivery_plan:request.presentation_candidate?.presentation?.event_delivery_plan??project?.spatial3d?.event_delivery_plan??null,initial_animation_root:runtimeEvidence?.evidence?.presentation_animation_root??null,asset_cache:request.asset_cache??{enabled:true,max_bytes:null},asset_payloads:presentationAssets},spatial_replay:{bundle_file:runtimeEvidence?.spatial_replay_bundle?'spatial-replay-bundle.json':null,result_file:runtimeEvidence?.spatial_replay_result?'spatial-replay-result.json':null,bundle_root:runtimeEvidence?.evidence?.spatial_replay_bundle_root??null,result_root:runtimeEvidence?.evidence?.spatial_replay_result_root??null,deterministic:runtimeEvidence?.evidence?.spatial_replay_deterministic??null,checkpoint_count:runtimeEvidence?.evidence?.spatial_replay_checkpoint_count??0},gpu_runtime:{preferred_backend:'webgpu',fallback_backend:'canvas2d',frame_plan_root:runtimeEvidence?.gpu_frame_plan?.framePlanRoot??runtimeEvidence?.gpu_frame_summary?.frame_plan_root??null,frame_summary_root:runtimeEvidence?.gpu_frame_summary?.summary_root??null,viewport_manifest_root:runtimeEvidence?.gpu_viewport_manifest?.manifest_root??null},gpu_frame_plan:runtimeEvidence?.gpu_frame_plan??null,gpu_frame_summary:runtimeEvidence?.gpu_frame_summary??null,gpu_viewport_manifest:runtimeEvidence?.gpu_viewport_manifest??null};}
function writeUIInputManifest(dir,payload){if(payload?.ui_input_runtime)writeJson(path.join(dir,'ui-input.manifest.json'),payload.ui_input_runtime.manifest);}
function embeddedHtml({project,request,identity,assetManifest,publicAssetManifest,target,runtimeEvidence,assetDatabase}){const assets=assetRuntimeMap(assetManifest,{embedded:true}),audioTargetPlan=project.audio_target?compileAudioTargetPlan(project.audio_target,{records:assetManifest.records,embedded:true}):null,runtimeProject=audioTargetPlan?{...project,audio_target_plan:audioTargetPlan}:project,presentationAssets=materializeSpatialAssetPayloads(request.presentation_candidate,{embedded:true}),payload=runtimePayload(runtimeProject,assets,request,identity,target,runtimeEvidence,assetDatabase,publicAssetManifest,presentationAssets);return renderIndexHtml({title:request.app.title,inline:true,runtimeSource:buildBrowserBehaviorRuntime(),rsrRuntimeSource:buildBrowserRSRRuntime(),spatial3dRuntimeSource:buildBrowserSpatial3DRuntime(),gpuRuntimeSource:buildBrowserVSRRuntime(),uiInputRuntimeSource:buildBrowserUIInputRuntime(),gameSource:buildGameSource(),payload});}
...
const spatialBundleFile=path.join(root,'spatial-replay-bundle.json');
const spatialResultFile=path.join(root,'spatial-replay-result.json');
const spatialBundle=fs.existsSync(spatialBundleFile)?JSON.parse(fs.readFileSync(spatialBundleFile,'utf8')):null;
const spatialArtifact=fs.existsSync(spatialResultFile)?JSON.parse(fs.readFileSync(spatialResultFile,'utf8')):null;
const spatialVerification=spatialBundle?verifySpatialReplayBundle(spatialBundle):{valid:true,deterministic:true,skipped:true};
const spatialReplay=spatialBundle&&spatialVerification.valid?replaySpatialReplayBundle(spatialBundle):null;
const spatialReplayOk=!spatialBundle||(spatialVerification.valid&&spatialVerification.deterministic&&spatialReplay?.replay_root===evidence.spatial_replay_result_root&&spatialArtifact?.replay_root===spatialReplay.replay_root);
const result={format:'reality-build.replay-verification.v0.3',build_id:evidence.build_id,evidence_root:evidence.evidence_root,expected_replay_root:evidence.replay_root,actual_replay_root:replay.replay_root,deterministic:replay.deterministic,expected_spatial_state_root:evidence.spatial_final_state_root,actual_spatial_state_root:spatialStateRoot,spatial_deterministic:evidence.spatial_deterministic===true&&spatialStateRoot===evidence.spatial_final_state_root,spatial_replay_bundle_root:evidence.spatial_replay_bundle_root??null,spatial_replay_result_root:evidence.spatial_replay_result_root??null,spatial_replay_deterministic:spatialBundle?spatialVerification.deterministic:null,spatial_replay_ok:spatialReplayOk,ok:replay.deterministic&&replay.replay_root===evidence.replay_root&&evidence.spatial_deterministic===true&&spatialStateRoot===evidence.spatial_final_state_root&&spatialReplayOk};
...
function headlessServerSource({networkEnabled=false}={}){
  ...
  const networkNodeImport=networkEnabled?"import{NetworkSessionCheckpointStore}from'@taowind/reality-network-runtime/node';\n":'';
...
const networkCheckpointPath=String(process.env.RNCS_NETWORK_CHECKPOINT_PATH??'').trim();
const networkCheckpointStore=networkCheckpointPath?new NetworkSessionCheckpointStore({filePath:networkCheckpointPath}):null;
const requireNetworkCheckpointStore=()=>{if(!networkCheckpointStore)throw Object.assign(new Error('NETWORK_CHECKPOINT_PATH_REQUIRED'),{code:'NETWORK_CHECKPOINT_PATH_REQUIRED'});return networkCheckpointStore;};
`:'';
...
if(req.method==='POST'&&u.pathname==='/network/authority/checkpoint'){await ensureNetworkAuthoritySession();const store=requireNetworkCheckpointStore(),checkpoint=networkRuntime.createCheckpoint({sessionId:networkAuthoritySessionId}),receipt=await store.save(checkpoint);return send(res,200,{status:'COMMITTED_CANDIDATE',checkpointRoot:checkpoint.checkpointRoot,tick:checkpoint.tick,receipt});}
if(req.method==='POST'&&u.pathname==='/network/authority/recover'){const store=requireNetworkCheckpointStore(),recovered=await store.recover();if(recovered.status!=='RECOVERED')return send(res,409,{error:'NETWORK_CHECKPOINT_UNAVAILABLE',status:recovered.status,source:recovered.source,diagnostics:recovered.diagnostics});const restored=await networkRuntime.createSessionFromCheckpoint({checkpoint:recovered.checkpoint,compilation:networkCompilation});networkAuthorityReady=Promise.resolve(restored);return send(res,200,{status:'RECOVERED_CANDIDATE',source:recovered.source,checkpointRoot:recovered.checkpoint.checkpointRoot,tick:recovered.checkpoint.tick,receipt:recovered.receipt,diagnostics:recovered.diagnostics});}
`:'';
...
import{UnifiedManufacturingSession}from'@taowind/reality-studio-native';
${networkImport}${networkNodeImport}const root=path.dirname(fileURLToPath(import.meta.url));
const project=JSON.parse(fs.readFileSync(path.join(root,'project.json'),'utf8'));
const evidence=JSON.parse(fs.readFileSync(path.join(root,'runtime-evidence.json'),'utf8'));
const session=new UnifiedManufacturingSession(project,{sessionId:'build-runtime:'+evidence.build_id});
session.createRuntimeCheckpoint('build-initial');
...
export function buildWebRelease({project,request,identity,bakeDir,outRoot,assetManifest,publicAssetManifest,runtimeEvidence,assetDatabase}){
  ...
  const presentationAssets=materializeSpatialAssetPayloads(request.presentation_candidate,{targetRoot:dir});writeSpatialAssetPayloadManifest(dir,presentationAssets);
  const assets=assetRuntimeMap(assetManifest,{embedded:false}),audioTargetPlan=project.audio_target?compileAudioTargetPlan(project.audio_target,{records:assetManifest.records,embedded:false}):null,runtimeProject=audioTargetPlan?{...project,audio_target_plan:audioTargetPlan}:project,payload=runtimePayload(runtimeProject,assets,request,identity,'web-release',runtimeEvidence,assetDatabase,publicAssetManifest,presentationAssets);writeRuntimeEvidenceFiles(dir,runtimeEvidence);writeTargetAssetEvidence(dir,assetDatabase,publicAssetManifest);writeUIInputManifest(dir,payload);if(audioTargetPlan)writeJson(path.join(dir,'audio-target-plan.json'),audioTargetPlan);
  writeText(path.join(dir,'runtime.js'),buildBrowserBehaviorRuntime());writeText(path.join(dir,'rsr-runtime.js'),buildBrowserRSRRuntime());writeText(path.join(dir,'spatial3d-runtime.js'),buildBrowserSpatial3DRuntime());writeText(path.join(dir,'vsr-runtime.js'),buildBrowserVSRRuntime());writeText(path.join(dir,'ui-input-runtime.js'),buildBrowserUIInputRuntime());writeText(path.join(dir,'game.js'),buildGameSource());writeText(path.join(dir,'build-data.js'),buildDataSource(payload));
  ...
  writeText(path.join(dir,'service-worker.js'),`const CACHE='${identity.build_key.slice(0,20)}';const FILES=${JSON.stringify(['./','./index.html','./runtime.js','./rsr-runtime.js','./spatial3d-runtime.js','./vsr-runtime.js','./ui-input-runtime.js','./game.js','./build-data.js','./manifest.webmanifest',...(payload.ui_input_runtime?['./ui-input.manifest.json']:[]),'./runtime-evidence.json','./runtime-timeline.json','./runtime-replay.json','./runtime-checkpoint.json','./spatial-initial-snapshot.json','./spatial-world.json','./spatial-scene.json','./spatial-frame-plan.json','./spatial-snapshot.json','./spatial-causal-delta.json','./spatial-runtime.manifest.json',...(runtimeEvidence?.spatial_replay_bundle?['./spatial-replay-bundle.json','./spatial-replay-result.json']:[]),'./gpu-frame-plan.json','./gpu-frame-summary.json','./gpu-viewport.manifest.json',...(runtimeEvidence?.navigation_manifest?['./tilemap-navigation.manifest.json']:[]),...(presentationAssets?['./spatial-asset-payload-manifest.json',...presentationAssets.files.map(file=>'./'+file)]:[]),...Object.values(assetManifest.records).flatMap(r=>r.files.map(f=>'./'+f.store_path))])};self.addEventListener('install',e=>e.waitUntil(caches.open(CACHE).then(c=>c.addAll(FILES))));self.addEventListener('fetch',e=>e.respondWith(caches.match(e.request).then(r=>r||fetch(e.request))));\n`);
...
export function buildReplayBundle({project,request,identity,outRoot,runtimeEvidence,assetDatabase,publicAssetManifest}){
  const dir=emptyDir(path.join(outRoot,'replay-bundle'));writeJson(path.join(dir,'project.json'),artifactProject(project));writeRuntimeEvidenceFiles(dir,runtimeEvidence);writeTargetAssetEvidence(dir,assetDatabase,publicAssetManifest);writeText(path.join(dir,'verify-replay.mjs'),replayVerifierSource());const spatialReplayFiles=runtimeEvidence.spatial_replay_bundle?['spatial-replay-bundle.json','spatial-replay-result.json']:[];writeJson(path.join(dir,'replay-manifest.json'),seal({format:'reality-build.replay-bundle-manifest.v0.3',version:'0.3.0-alpha.1',build_id:identity.build_id,project_root:project.project_root,entry:'verify-replay.mjs',runtime_evidence:runtimeEvidenceSummary(runtimeEvidence),spatial_runtime_manifest_root:runtimeEvidence.spatial_runtime_manifest?.manifest_root??null,spatial_replay_bundle_root:runtimeEvidence.evidence.spatial_replay_bundle_root??null,spatial_replay_result_root:runtimeEvidence.evidence.spatial_replay_result_root??null,gpu_frame_plan_root:runtimeEvidence.evidence.gpu_frame_plan_root,gpu_frame_summary_root:runtimeEvidence.evidence.gpu_frame_summary_root,gpu_viewport_manifest_root:runtimeEvidence.evidence.gpu_viewport_manifest_root,navigation_manifest_root:runtimeEvidence.navigation_manifest?.manifest_root??null,asset_database_root:assetDatabase?.evidence?.database_root??null,files:['project.json','runtime-evidence.json','runtime-timeline.json','runtime-replay.json','runtime-checkpoint.json','spatial-snapshot.json','spatial-causal-delta.json','spatial-runtime.manifest.json',...spatialReplayFiles,'gpu-frame-plan.json','gpu-frame-summary.json','gpu-viewport.manifest.json',...(runtimeEvidence.navigation_manifest?['tilemap-navigation.manifest.json']:[]),'verify-replay.mjs']},'manifest_root'));return{dir,receipt:manifestForTarget(dir,'replay-bundle',identity)};
}
...
export function generateAndroidProject({dir,project,request,identity,assetManifest,publicAssetManifest,runtimeEvidence,assetDatabase,target='android-project'}){
  emptyDir(dir);const appDir=path.join(dir,'app'),assetsDir=ensureDir(path.join(appDir,'src/main/assets')),uiInputRuntime=createBuildUIInputRuntime(project);
  writeText(path.join(assetsDir,'index.html'),embeddedHtml({project,request,identity,assetManifest,publicAssetManifest,target,runtimeEvidence,assetDatabase}));writeUIInputManifest(assetsDir,{ui_input_runtime:uiInputRuntime});writeRuntimeEvidenceFiles(assetsDir,runtimeEvidence);writeTargetAssetEvidence(assetsDir,assetDatabase,publicAssetManifest);
...
export function buildAndroidApk(ctx){
  ...
  const r=spawnSync(invocation.command,invocation.args,{cwd:projectDir,encoding:'utf8',windowsHide:true,windowsVerbatimArguments:process.platform==='win32'&&path.basename(invocation.command).toLowerCase()==='cmd.exe',timeout:10*60*1000,env:process.env});
  ...
  if(tools.android_sdk.apksigner){const invocation=commandInvocation(tools.android_sdk.apksigner,['verify','--verbose',apkFile]);const vr=spawnSync(invocation.command,invocation.args,{encoding:'utf8',windowsHide:true,windowsVerbatimArguments:process.platform==='win32'&&path.basename(invocation.command).toLowerCase()==='cmd.exe',env:process.env});signatureVerification={status:vr.status===0?'verified':'failed',output:String(vr.stdout||vr.stderr||'').trim().slice(0,2000)};if(vr.status!==0){fs.rmSync(projectDir,{recursive:true,force:true});throw new BuildError('ANDROID_APK_SIGNATURE_INVALID',signatureVerification.output);}}
