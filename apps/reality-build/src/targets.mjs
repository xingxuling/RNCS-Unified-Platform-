...
import {findCommand,inspectToolchains} from './preflight.mjs';
...
function runtimePayload(project,assets,request,identity,target){return{project,assets,build:{format:'reality-build.runtime.v0.2',build_id:identity.build_id,build_key:identity.build_key,target,mode:request.mode,quality_profile:request.quality_profile,app:request.app,build_time:request.build_time}};}
function embeddedHtml({project,request,identity,assetManifest,target}){const assets=assetRuntimeMap(assetManifest,{embedded:true});const payload=runtimePayload(project,assets,request,identity,target);return renderIndexHtml({title:request.app.title,inline:true,runtimeSource:buildBrowserBehaviorRuntime(),gameSource:buildGameSource(),payload});}

export function buildWebRelease({project,request,identity,bakeDir,outRoot,assetManifest}){
  const dir=emptyDir(path.join(outRoot,'web-release'));copyBakedAssets(assetManifest,bakeDir,dir);
  const assets=assetRuntimeMap(assetManifest,{embedded:false});const payload=runtimePayload(project,assets,request,identity,'web-release');
...
export function buildWebSingle({project,request,identity,outRoot,assetManifest}){
  const dir=emptyDir(path.join(outRoot,'web-single'));const file=path.join(dir,`${safeName(request.app.title)}_单文件版.html`);writeText(file,embeddedHtml({project,request,identity,assetManifest,target:'web-single'}));
  return{dir,file,receipt:manifestForTarget(dir,'web-single',identity)};
}

export function buildWindowsPortable({project,request,identity,outRoot,assetManifest}){
  const dir=emptyDir(path.join(outRoot,'windows-portable')),htmlName=`${safeName(request.app.title)}.html`;
  writeText(path.join(dir,htmlName),embeddedHtml({project,request,identity,assetManifest,target:'windows-portable'}));
...
export function buildWindowsNative({project,request,identity,outRoot,assetManifest}){
  const dir=emptyDir(path.join(outRoot,'windows-native')),exeName=`${safeName(request.app.title)}.exe`,outFile=path.join(dir,exeName);
  const html=embeddedHtml({project,request,identity,assetManifest,target:'windows-native'});
...
export function generateAndroidProject({dir,project,request,identity,assetManifest,target='android-project'}){
  emptyDir(dir);const appDir=path.join(dir,'app'),assetsDir=ensureDir(path.join(appDir,'src/main/assets'));
  writeText(path.join(assetsDir,'index.html'),embeddedHtml({project,request,identity,assetManifest,target}));
...
export function buildAndroidApk(ctx){
  ...
  const r=spawnSync(gradle,['--no-daemon','app:assembleDebug'],{cwd:projectDir,encoding:'utf8',windowsHide:true,timeout:10*60*1000,env:process.env});
...
export const TARGET_BUILDERS={'web-release':buildWebRelease,'web-single':buildWebSingle,'windows-portable':buildWindowsPortable,'windows-native':buildWindowsNative,'android-project':buildAndroidProject,'android-apk':buildAndroidApk};
