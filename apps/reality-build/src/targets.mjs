...
import {buildBrowserBehaviorRuntime,buildBrowserRSRRuntime,buildBrowserVSRRuntime,buildGameSource,renderIndexHtml} from './runtime-template.mjs';
...
export function buildWebRelease({project,request,identity,bakeDir,outRoot,assetManifest,publicAssetManifest,runtimeEvidence,assetDatabase}){
  ...
  writeText(path.join(dir,'runtime.js'),buildBrowserBehaviorRuntime());writeText(path.join(dir,'rsr-runtime.js'),buildBrowserRSRRuntime());writeText(path.join(dir,'vsr-runtime.js'),buildBrowserVSRRuntime());writeText(path.join(dir,'game.js'),buildGameSource());writeText(path.join(dir,'build-data.js'),buildDataSource(payload));
  ...
  writeText(path.join(dir,'service-worker.js'),`const CACHE='${identity.build_key.slice(0,20)}';const FILES=${JSON.stringify(['./','./index.html','./runtime.js','./rsr-runtime.js','./vsr-runtime.js','./game.js','./build-data.js','./manifest.webmanifest','./runtime-evidence.json','./runtime-timeline.json','./runtime-replay.json','./runtime-checkpoint.json','./spatial-snapshot.json','./spatial-causal-delta.json','./spatial-runtime.manifest.json','./gpu-frame-plan.json','./gpu-frame-summary.json','./gpu-viewport.manifest.json',...(runtimeEvidence?.navigation_manifest?['./tilemap-navigation.manifest.json']:[]),...Object.values(assetManifest.records).flatMap(r=>r.files.map(f=>'./'+f.store_path))])};self.addEventListener('install',e=>e.waitUntil(caches.open(CACHE).then(c=>c.addAll(FILES))));self.addEventListener('fetch',e=>e.respondWith(caches.match(e.request).then(r=>r||fetch(e.request))));\n`);
