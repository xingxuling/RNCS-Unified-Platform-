...
import {assetRuntimeMap,copyBakedAssets} from './assets.mjs';
import {runtimeEvidenceSummary} from './runtime-evidence.mjs';
import {buildBrowserBehaviorRuntime,buildGameSource,renderIndexHtml} from './runtime-template.mjs';
...
function replayVerifierSource(){return `import fs from'node:fs';import path from'node:path';import{fileURLToPath}from'node:url';import{UnifiedManufacturingSession}from'@taowind/reality-studio-native';const root=path.dirname(fileURLToPath(import.meta.url)),project=JSON.parse(fs.readFileSync(path.join(root,'project.json'),'utf8')),evidence=JSON.parse(fs.readFileSync(path.join(root,'runtime-evidence.json'),'utf8')),session=new UnifiedManufacturingSession(project,{sessionId:'build-runtime:'+evidence.build_id});session.createRuntimeCheckpoint('build-initial');for(const input of evidence.trace??[])session.step(input);const replay=session.replayRuntime({verify:true}),result={format:'reality-build.replay-verification.v0.1',build_id:evidence.build_id,evidence_root:evidence.evidence_root,expected_replay_root:evidence.replay_root,actual_replay_root:replay.replay_root,deterministic:replay.deterministic,ok:replay.deterministic&&replay.replay_root===evidence.replay_root};console.log(JSON.stringify(result,null,2));if(!result.ok)process.exitCode=1;\n`;}
function headlessServerSource(){return `import fs from'node:fs';import path from'node:path';import{fileURLToPath}from'node:url';import{createServer}from'node:http';import{UnifiedManufacturingSession}from'@taowind/reality-studio-native';const root=path.dirname(fileURLToPath(import.meta.url)),project=JSON.parse(fs.readFileSync(path.join(root,'project.json'),'utf8')),evidence=JSON.parse(fs.readFileSync(path.join(root,'runtime-evidence.json'),'utf8')),session=new UnifiedManufacturingSession(project,{sessionId:'build-runtime:'+evidence.build_id});session.createRuntimeCheckpoint('build-initial');for(const input of evidence.trace??[])session.step(input);const send=(res,status,payload)=>{res.writeHead(status,{'content-type':'application/json; charset=utf-8','access-control-allow-origin':'*'});res.end(JSON.stringify(payload));};const body=req=>new Promise((resolve,reject)=>{let data='';req.on('data',chunk=>data+=chunk);req.on('end',()=>{try{resolve(data?JSON.parse(data):{})}catch(error){reject(error)}});req.on('error',reject)});const server=createServer(async(req,res)=>{try{const u=new URL(req.url??'/', 'http://127.0.0.1');if(req.method==='GET'&&u.pathname==='/health')return send(res,200,{status:'healthy',format:'reality-build.headless-server.v0.1',build_id:evidence.build_id,project_root:project.project_root,replay_root:evidence.replay_root,deterministic:evidence.deterministic,tick:session.behavior.runtime.state.tick});if(req.method==='GET'&&u.pathname==='/inspect')return send(res,200,session.inspect());if(req.method==='GET'&&u.pathname==='/replay')return send(res,200,session.replayRuntime({verify:true}));if(req.method==='POST'&&u.pathname==='/step')return send(res,200,session.step((await body(req)).input??{}));return send(res,404,{error:'NOT_FOUND'});}catch(error){return send(res,400,{error:error.code??'HEADLESS_SERVER_ERROR',message:error.message})}});server.listen(Number(process.env.PORT??4174),'127.0.0.1',()=>console.log('http://127.0.0.1:'+Number(process.env.PORT??4174)));\n`;}

export function buildWebRelease({project,request,identity,bakeDir,outRoot,assetManifest,runtimeEvidence}){
  const dir=emptyDir(path.join(outRoot,'web-release'));copyBakedAssets(assetManifest,bakeDir,dir);
  const assets=assetRuntimeMap(assetManifest,{embedded:false});const payload=runtimePayload(project,assets,request,identity,'web-release',runtimeEvidence);writeRuntimeEvidenceFiles(dir,runtimeEvidence);
  writeText(path.join(dir,'runtime.js'),buildBrowserBehaviorRuntime());writeText(path.join(dir,'game.js'),buildGameSource());writeText(path.join(dir,'build-data.js'),buildDataSource(payload));
  ...
  writeText(path.join(dir,'service-worker.js'),`const CACHE='${identity.build_key.slice(0,20)}';const FILES=${JSON.stringify(['./','./index.html','./runtime.js','./game.js','./build-data.js','./manifest.webmanifest','./runtime-evidence.json','./runtime-timeline.json','./runtime-replay.json','./runtime-checkpoint.json',...Object.values(assetManifest.records).flatMap(r=>r.files.map(f=>'./'+f.store_path))])};self.addEventListener('install',e=>e.waitUntil(caches.open(CACHE).then(c=>c.addAll(FILES))));self.addEventListener('fetch',e=>e.respondWith(caches.match(e.request).then(r=>r||fetch(e.request))));\n`);
...
export function buildWebSingle({project,request,identity,outRoot,assetManifest,runtimeEvidence}){
  const dir=emptyDir(path.join(outRoot,'web-single'));const file=path.join(dir,`${safeName(request.app.title)}_单文件版.html`);writeText(file,embeddedHtml({project,request,identity,assetManifest,target:'web-single',runtimeEvidence}));
  return{dir,file,receipt:manifestForTarget(dir,'web-single',identity)};
}

export function buildWindowsPortable({project,request,identity,outRoot,assetManifest,runtimeEvidence}){
  const dir=emptyDir(path.join(outRoot,'windows-portable')),htmlName=`${safeName(request.app.title)}.html`;
  writeText(path.join(dir,htmlName),embeddedHtml({project,request,identity,assetManifest,target:'windows-portable',runtimeEvidence}));writeRuntimeEvidenceFiles(dir,runtimeEvidence);
...
export function buildWindowsNative({project,request,identity,outRoot,assetManifest,runtimeEvidence}){
  const dir=emptyDir(path.join(outRoot,'windows-native')),exeName=`${safeName(request.app.title)}.exe`,outFile=path.join(dir,exeName);
  const html=embeddedHtml({project,request,identity,assetManifest,target:'windows-native',runtimeEvidence});writeRuntimeEvidenceFiles(dir,runtimeEvidence);
...
export function buildHeadlessServer({project,request,identity,outRoot,runtimeEvidence}){
  const dir=emptyDir(path.join(outRoot,'headless-server'));writeJson(path.join(dir,'project.json'),project);writeRuntimeEvidenceFiles(dir,runtimeEvidence);writeText(path.join(dir,'server.mjs'),headlessServerSource());writeText(path.join(dir,'verify-replay.mjs'),replayVerifierSource());writeJson(path.join(dir,'package.json'),{name:`${safeName(request.app.title,'reality-app').toLowerCase().replace(/[^a-z0-9]+/g,'-')}-headless-server`,version:request.app.version_name,private:true,type:'module',scripts:{start:'node server.mjs',verify:'node verify-replay.mjs'},dependencies:{'@taowind/reality-studio-native':'^1.6.0-alpha.1'}});writeJson(path.join(dir,'server-manifest.json'),seal({format:'reality-build.headless-server-manifest.v0.1',version:'0.1.0-alpha.1',build_id:identity.build_id,project_root:project.project_root,entry:'server.mjs',verification_entry:'verify-replay.mjs',health_endpoint:'/health',inspect_endpoint:'/inspect',step_endpoint:'/step',replay_endpoint:'/replay',runtime_evidence:runtimeEvidenceSummary(runtimeEvidence)},'manifest_root'));writeText(path.join(dir,'README.md'),`# ${request.app.title} Headless Server\n\nRun npm install, then npm start. The server exposes /health, /inspect, /step, and /replay.\n`);return{dir,receipt:manifestForTarget(dir,'headless-server',identity)};
}

export function buildReplayBundle({project,request,identity,outRoot,runtimeEvidence}){
  const dir=emptyDir(path.join(outRoot,'replay-bundle'));writeJson(path.join(dir,'project.json'),project);writeRuntimeEvidenceFiles(dir,runtimeEvidence);writeText(path.join(dir,'verify-replay.mjs'),replayVerifierSource());writeJson(path.join(dir,'replay-manifest.json'),seal({format:'reality-build.replay-bundle-manifest.v0.1',version:'0.1.0-alpha.1',build_id:identity.build_id,project_root:project.project_root,entry:'verify-replay.mjs',runtime_evidence:runtimeEvidenceSummary(runtimeEvidence),files:['project.json','runtime-evidence.json','runtime-timeline.json','runtime-replay.json','runtime-checkpoint.json','verify-replay.mjs']},'manifest_root'));return{dir,receipt:manifestForTarget(dir,'replay-bundle',identity)};
}
...
export function generateAndroidProject({dir,project,request,identity,assetManifest,runtimeEvidence,target='android-project'}){
  emptyDir(dir);const appDir=path.join(dir,'app'),assetsDir=ensureDir(path.join(appDir,'src/main/assets'));
  writeText(path.join(assetsDir,'index.html'),embeddedHtml({project,request,identity,assetManifest,target,runtimeEvidence}));writeRuntimeEvidenceFiles(assetsDir,runtimeEvidence);
