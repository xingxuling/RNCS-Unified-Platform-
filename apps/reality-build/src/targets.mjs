...
function writeRuntimeEvidenceFiles(dir,runtimeEvidence){if(!runtimeEvidence)return;writeJson(path.join(dir,'runtime-evidence.json'),runtimeEvidence.evidence);writeJson(path.join(dir,'runtime-timeline.json'),runtimeEvidence.timeline);writeJson(path.join(dir,'runtime-replay.json'),runtimeEvidence.replay);writeJson(path.join(dir,'runtime-checkpoint.json'),runtimeEvidence.checkpoint);writeJson(path.join(dir,'spatial-snapshot.json'),runtimeEvidence.spatial_snapshot);writeJson(path.join(dir,'spatial-causal-delta.json'),runtimeEvidence.spatial_causal_delta);writeJson(path.join(dir,'spatial-runtime.manifest.json'),runtimeEvidence.spatial_runtime_manifest);if(runtimeEvidence.navigation_manifest)writeJson(path.join(dir,'tilemap-navigation.manifest.json'),runtimeEvidence.navigation_manifest);}
...
export function buildWebRelease({project,request,identity,bakeDir,outRoot,assetManifest,publicAssetManifest,runtimeEvidence,assetDatabase}){
  ...
  writeText(path.join(dir,'service-worker.js'),`const CACHE='${identity.build_key.slice(0,20)}';const FILES=${JSON.stringify(['./','./index.html','./runtime.js','./game.js','./build-data.js','./manifest.webmanifest','./runtime-evidence.json','./runtime-timeline.json','./runtime-replay.json','./runtime-checkpoint.json','./spatial-snapshot.json','./spatial-causal-delta.json','./spatial-runtime.manifest.json',...(runtimeEvidence?.navigation_manifest?['./tilemap-navigation.manifest.json']:[]),...Object.values(assetManifest.records).flatMap(r=>r.files.map(f=>'./'+f.store_path))])};self.addEventListener('install',e=>e.waitUntil(caches.open(CACHE).then(c=>c.addAll(FILES))));self.addEventListener('fetch',e=>e.respondWith(caches.match(e.request).then(r=>r||fetch(e.request))));\n`);
