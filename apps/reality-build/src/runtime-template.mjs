...
export function buildBrowserVSRRuntime(){
  ...
  return `${source}\nwindow.__RNCSVSR__={VSRRealtimeWebGPUExecutor,normalizeRealtimeWebGPUFrame,probeRealtimeWebGPU,verifyRealtimeGPUReceipt,verifyRealtimeWebGPUFrame,VSR_REALTIME_WEBGPU_VERSION};window.dispatchEvent?.(new Event('rncs-vsr-ready'));\n`;
}
...
const keys=new Set(),images={},particles=[];let audioCtx=null,runtime=null,lastTick=0,acc=0,last=performance.now(),paused=false,gpuExecutor=null,gpuReceipt=null,gpuReady=false,gpuRenderInFlight=false,gpuError='';
...
function renderGPU(time){
 ...
 void gpuExecutor.render(DATA.gpu_frame_plan,time/1000).then(receipt=>{gpuReceipt=receipt;}).catch(error=>{gpuError=String(error?.message??error);gpuReady=false;canvas.hidden=false;gpuCanvas.hidden=true;statusEl.textContent='Canvas 2D';}).finally(()=>{gpuRenderInFlight=false;});
}
...
function frame(now){const dt=Math.min(100,now-last);last=now;acc+=dt;const step=1000/program.tick_rate;while(acc>=step){tick();acc-=step;}if(gpuReady)renderGPU(now);else render();requestAnimationFrame(frame)}requestAnimationFrame(frame);
window.__RNCS_GAME__={get runtime(){return runtime},get gpu(){return{ready:gpuReady,receipt:gpuReceipt,error:gpuError}},reset:createRuntime,project,build};
})();`}

export function renderIndexHtml({title,inline=false,runtimeSource='',gpuRuntimeSource='',gameSource='',payload=null,manifestPath='manifest.webmanifest'}){
  ...
  const scripts=inline?`<script>${runtimeSource.replaceAll('</script>','<\\/script>')}</script><script>${gpuRuntimeSource.replaceAll('</script>','<\\/script>')}</script><script>${gameSource.replaceAll('</script>','<\\/script>')}</script>`:`<script src="runtime.js"></script><script src="vsr-runtime.js"></script><script src="game.js"></script>`;
