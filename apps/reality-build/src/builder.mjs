...
export function buildProject(input={}){
  ...
  const targets=[];for(const target of request.targets){t=performance.now();const builder=TARGET_BUILDERS[target];const result=builder({project,request,identity,bakeDir,outRoot,assetManifest,runtimeEvidence});targets.push({target,target_root:result.receipt.target_root,files:result.receipt.files.length,profile:TARGET_PROFILES[target]});mark(`target:${target}`,t,{files:result.receipt.files.length});}
  ...
  const integrity=seal({format:'reality-build.integrity-manifest.v0.2',version:'0.2.0-alpha.1',build_id:identity.build_id,build_key:identity.build_key,graph_root:graph.graph_root,runtime_evidence_root:runtimeEvidence.evidence.evidence_root,targets:targets.map(t=>({target:t.target,target_root:t.target_root,files:t.files}))},'integrity_root');writeJson(path.join(outRoot,'integrity-manifest.json'),integrity);
  const coreFiles=['build-graph.json','validation.json','build-preflight.json','runtime-evidence.json','runtime-timeline.json','runtime-replay.json','runtime-checkpoint.json','asset-manifest.json','integrity-manifest.json'].map(rel=>({path:rel,size:fs.statSync(path.join(outRoot,rel)).size,sha256:sha256File(path.join(outRoot,rel))}));
  const receipt=seal({format:'reality-build.receipt.v0.2',version:'0.2.0-alpha.1',build_id:identity.build_id,build_key:identity.build_key,project_id:project.identity.project_id,project_root:identity.project_root,request_root:request.request_root,semantic_request_root:identity.semantic_request_root,graph_root:graph.graph_root,preflight_root:preflight.preflight_root,runtime_evidence_root:runtimeEvidence.evidence.evidence_root,runtime_timeline_root:runtimeEvidence.timeline.timeline_root,runtime_replay_root:runtimeEvidence.replay.replay_root,runtime_deterministic:runtimeEvidence.replay.deterministic,mode:request.mode,quality_profile:request.quality_profile,app:clone(request.app),build_time:request.build_time,targets,core_files:coreFiles,integrity_root:integrity.integrity_root,deterministic_outputs:request.policy.deterministic,environment_evidence:true,status:'built'},'receipt_root');writeJson(path.join(outRoot,'build-receipt.json'),receipt);
