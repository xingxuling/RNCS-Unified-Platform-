...
- Android target generation now emits parseable independent `debug`/`release` Gradle closures, Windows `.bat` toolchains are invoked through the shared command adapter, and the local `android-apk` path records automatic `apksigner` verification; this closes host APK build evidence, not WebView/device or release-signing evidence.
