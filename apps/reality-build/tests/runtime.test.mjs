...
test('浏览器 VSR 3D 入口无 Node 依赖并暴露真实执行器',()=>{const source=buildBrowserSpatial3DRuntime();assert.doesNotMatch(source,/node:/);assert.doesNotMatch(source,/^\s*export\s/m);assert.match(source,/var VSRSpatial3D/);assert.match(source,/VSRSpatialWebGPUExecutor/);assert.match(source,/compileSpatialFrame/);assert.match(source,/verifySpatialFrame/);assert.match(source,/__RNCS3D__/);assert.doesNotThrow(()=>new Function(source))});
test('3D 游戏宿主复用 VSR 资产流送解析、字节加载并暴露诊断',()=>{assert.match(game,/resolveSpatialAssetStreaming/);assert.match(game,/VSRSpatialAssetStreamer/);assert.match(game,/loadSpatial3DAssets/);assert.match(game,/assetStreaming:spatial3dAssetStreaming/);assert.match(game,/asset_streaming_root/);assert.match(game,/asset_payload_receipt_root/);assert.match(game,/assetPayloadRequestedCount/);assert.match(game,/assetPayloadReadyCount/)});
