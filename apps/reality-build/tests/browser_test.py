...
with sync_playwright() as p:
    ...
    for mode in ['web-release','web-single','windows-portable']:
      ...
      outcome=page.evaluate('''(expectNavigation)=>{const g=window.__RNCS_GAME__;g.reset();const r=g.runtime,initial_gpu=g.gpuFrameSnapshot(),initial_spatial=structuredClone(g.spatial),navigation=expectNavigation?g.queryNavigation({x:24,y:32},{x:616,y:336}):null;let ticks=0;while(ticks<500&&!r.state.globals.victory&&!r.state.globals.defeat){const p=r.state.entities.player.variables,e=r.state.entities.enemy?.variables??{health:0,x:0};let input={move_left:false,move_right:false,attack:false,restart:false};if(!p.has_key)input.move_right=true;else if((e.health??0)>0){const d=Math.abs(p.x-e.x);if(d>56)input.move_right=p.x<e.x;else input.attack=true;}else input.move_right=true;g.step(input);ticks++;}return{ticks,victory:r.state.globals.victory,defeat:r.state.globals.defeat,score:r.state.globals.score,state_root:r.stateRoot(),player:r.state.entities.player.variables,enemy:r.state.entities.enemy?.variables??null,navigation,spatial:{initial:initial_spatial,final:structuredClone(g.spatial)},dynamic_gpu:{initial:initial_gpu,final:g.gpuFrameSnapshot()}};}''',expect_navigation)
      page.wait_for_timeout(500);runtime_info=page.evaluate('''()=>{const g=window.__RNCS_GAME__;return{vsr_loaded:Boolean(window.__RNCSVSR__),rsr_loaded:Boolean(window.__RNCSRSR__),navigator_gpu:Boolean(navigator.gpu),gpu_ready:Boolean(g?.gpu?.ready),gpu_error:g?.gpu?.error??'',gpu_receipt:g?.gpu?.receipt?.format??null,spatial:g?.spatial??null}}''');assert outcome['victory'] is True, outcome;assert runtime_info['vsr_loaded'] is True, runtime_info;assert runtime_info['rsr_loaded'] is True, runtime_info;assert runtime_info['spatial']['verified'] is True, runtime_info;assert runtime_info['spatial']['binding_count'] > 0, runtime_info;assert not errors, errors
