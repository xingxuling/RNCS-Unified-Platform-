...
const nativePreflight=runBuildPreflight({request:request(['windows-native']),project});
test('当前环境可构建 Windows 原生目标',{skip:nativePreflight.status!=='ready'},()=>{assert.equal(nativePreflight.status,'ready')});
...
test('Doctor 识别 Windows build-tools 中的 apksigner.bat',()=>{const sdk=fs.mkdtempSync(path.join(os.tmpdir(),'rbf-sdk-tools-'));const buildTools=path.join(sdk,'build-tools','36.1.0');fs.mkdirSync(buildTools,{recursive:true});const signer=path.join(buildTools,'apksigner.bat');fs.writeFileSync(signer,'@echo off\r\n');const d=doctorReport({env:{PATH:'',ANDROID_SDK_ROOT:sdk},includePaths:true});assert.equal(d.toolchains.android_sdk.apksigner,signer)});
