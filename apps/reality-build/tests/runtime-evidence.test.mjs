...
test('release builds carry deterministic runtime evidence',()=>{
  ...
  assert.equal(verifySeal(evidenceA,'evidence_root'),true);assert.equal(evidenceA.evidence_root,evidenceB.evidence_root);assert.equal(evidenceA.timeline_root,evidenceB.timeline_root);assert.equal(evidenceA.replay_root,evidenceB.replay_root);assert.equal(evidenceA.spatial_deterministic,true);assert.equal(evidenceA.spatial_final_state_root,evidenceB.spatial_final_state_root);assert.equal(evidenceA.spatial_runtime_manifest_root,evidenceB.spatial_runtime_manifest_root);
  for(const file of ['runtime-evidence.json','runtime-timeline.json','runtime-replay.json','runtime-checkpoint.json','spatial-snapshot.json','spatial-causal-delta.json','spatial-runtime.manifest.json'])assert.ok(fs.existsSync(path.join(outA,file)));
  for(const file of ['runtime-evidence.json','runtime-timeline.json','runtime-replay.json','runtime-checkpoint.json','spatial-snapshot.json','spatial-causal-delta.json','spatial-runtime.manifest.json'])assert.ok(fs.existsSync(path.join(outA,'web-release',file)));
