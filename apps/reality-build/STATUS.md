...
- Candidate-only VSR presentation consumer: implemented for explicit World Body and generic Large World presentation candidates; Build evidence executes shared VSR asset-catalog resolution (`requested=4`, `missing=0`), and the Large World GLB candidate bundle now has a web-release content-addressed payload manifest plus browser VSR byte loading/hash verification (`catalog=36`, active request/ready=`33/33`, missing/failed=`0/0`); GLB scene import/binding, Android embedded runtime, network, and device execution remain open
