...
export async function startStudioServer({host='127.0.0.1',port=17608,dataDir=path.join(root,'output/server')}={}){
  ...
  const server=http.createServer(async(req,res)=>{
    try{
      ...
      if(req.method==='GET'&&u.pathname==='/api/anime-forge/sample')return send(res,200,{source:fs.readFileSync(path.resolve(root,'../../packages/integration/rcl-anime-production-bridge/examples/shenlinzhe-yanlv-editorial.rcl'),'utf8')});
