...
export async function startStudioServer({host='127.0.0.1',port=17608,dataDir=path.join(root,'output/server'),ffmpegPath=process.env.FFMPEG_PATH??'ffmpeg',ffprobePath=process.env.FFPROBE_PATH??'ffprobe',nativeCharacterSurgeryEvidenceDir=path.resolve(root,'../../evidence/anime-forge-phase6-1-morphogenesis-surgery-v0.1'),geometricTruthEvidenceDir=path.resolve(root,'../../evidence/anime-forge-phase6-3-geometric-truth-v0.1')}={}){
  ...
  const server=http.createServer(async(req,res)=>{
    try{
      ...
      if(req.method==='GET'&&u.pathname==='/api/anime-forge/geometric-truth')return send(res,200,animeForgeSessions.geometricTruthWorkspace());
      if(req.method==='POST'&&u.pathname==='/api/anime-forge/geometric-truth'){const b=await body(req),workspace=b.session_id?animeForgeSessions.get(b.session_id).geometricTruthWorkspace():animeForgeSessions.geometricTruthWorkspace();return send(res,200,{ok:true,...workspace});}
