...
export class UnifiedManufacturingSession{
  ...
  checkpoint({record=true}={}){if(this.spatial)this.project.spatial3d=deep(this.spatial.workspace);this.project=sealProject(this.project);if(record){this.history=this.history.slice(0,this.history_index+1);this.history.push(deep(this.project));this.history_index=this.history.length-1;}return this.inspect();}
  ...
  undo(){if(this.history_index<=0)return this.inspect();this.history_index--;this.project=deep(this.history[this.history_index]);const p=this.project.behavior.programs[this.project.behavior.active_program_id];this.behavior.replaceProgram(p,{preserveState:true,recordHistory:false});this.spatial=new SpatialStudioSession(this.project.spatial3d);this.record('editor.undo');return this.inspect();}
  redo(){if(this.history_index>=this.history.length-1)return this.inspect();this.history_index++;this.project=deep(this.history[this.history_index]);const p=this.project.behavior.programs[this.project.behavior.active_program_id];this.behavior.replaceProgram(p,{preserveState:true,recordHistory:false});this.spatial=new SpatialStudioSession(this.project.spatial3d);this.record('editor.redo');return this.inspect();}
  step(input={}){this.status='paused';this.stepNavigation();let actions=input;if(input?.raw||input?.keys||input?.gamepads||input?.touches){const frame=this.sampleInput(input.raw??input);actions=this.inputRuntime.actionBooleans(frame);for(const d of ['left','right','up','down'])if(frame.actions?.[`ui_${d}`]?.just_pressed)this.uiState.focus_id=moveUIFocus(this.compileUILayout(),this.uiState.focus_id,d);if(frame.actions?.ui_accept?.just_pressed&&this.uiState.focus_id){const node=this.activeUITree()?.nodes?.find(n=>n.ui_node_id===this.uiState.focus_id),action=node?.events?.pressed;if(action)actions[action]=true;}}this.behavior.step(actions);this.record('runtime.step',{tick:this.behavior.runtime.state.tick,input:actions});return this.inspect();}
  run({ticks=1,inputs=[]}={}){this.status='running';this.behavior.run({ticks,inputs});this.status='paused';this.record('runtime.run',{ticks,final_tick:this.behavior.runtime.state.tick});return this.inspect();}
  pause(){this.behavior.pause();this.status='paused';return this.inspect();}
  reset(){this.behavior.reset();this.status='editing';this.navigation.positions={};this.navigation.targets={};this.navigation.paths={};for(const n of activeScene(this.project).nodes??[])if(n.components?.navigation_agent)this.navigation.positions[n.node_id]={x:n.transform.x,y:n.transform.y,status:'idle',speed_milli:n.components.navigation_agent.speed_milli??2400};this.inputRuntime.reset();this.uiState.last_event=null;this.uiState.events=[];this.spatial.reset();this.record('runtime.reset');return this.inspect();}
  ...
  exportArtifacts(){
    ...
    const gpu=this.compileGPUFrame({serialized:false});const spatial=this.spatial.exportArtifacts();
