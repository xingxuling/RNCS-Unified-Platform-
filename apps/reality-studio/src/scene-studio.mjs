...
import {SequencerSession,createDefaultSequence} from './sequencer.mjs';
import {createEngineProposalInput,createRealityEngineSession} from '@taowind/reality-engine-session';
import {createAudioTargetProfile,verifyAudioTargetProfile} from '@taowind/audio-target-runtime';
...
export function createUnifiedProject({title='冰境试炼统一制造项目',program,assets=[],projectId=null,createdAt=null,audioTarget=null}={}){
  const p=program?.program_root?program:normalizeProgram(program);
  ...
  return sealMixed({
    ...
    scenes:[scene],
    assets:{format:'reality-studio.asset-catalog.v1.3',version:ASSET_CONTINUITY_VERSION,registry,order:Object.keys(registry),import_roots:[],last_audit_root:null,database:{format:ASSET_DATABASE_FORMAT,version:ASSET_DATABASE_VERSION,cache_namespace:'content-addressed',profiles:['runtime'],source_root_ids:[],last_plan_root:null,last_sync_root:null}},
    ...
    ...(audioTarget?{audio_target:deep(audioTarget)}:{})
  },'project_root');
}
...
export function validateUnifiedProject(project){
  ...
  if(project?.audio_target){const v=verifyAudioTargetProfile(project.audio_target);if(!v.valid)errors.push({code:'AUDIO_TARGET_PROFILE_INVALID',path:'audio_target',details:v});}
...
export function ensureUIInputProject(project){
  ...
  out.assets??={registry:{},order:[]};out.assets.format??='reality-studio.asset-catalog.v1.3';out.assets.version??=ASSET_CONTINUITY_VERSION;out.assets.import_roots??=[];out.assets.last_audit_root??=null;out.assets.database??={format:ASSET_DATABASE_FORMAT,version:ASSET_DATABASE_VERSION,cache_namespace:'content-addressed',profiles:['runtime'],source_root_ids:[],last_plan_root:null,last_sync_root:null};
...
export class UnifiedManufacturingSession{
  constructor(project,{sessionId=null,clock=now}={}){
    ...
    this.sequencer=new SequencerSession(this.project.sequencer??createDefaultSequence(this.project));
    this.project.sequencer=deep(this.sequencer.sequence);
    this.project=sealUnifiedProject(this.project,{touch:false});
  ...
  sequenceView(){return this.sequencer.inspect();}
  persistSequence(){this.project.sequencer=deep(this.sequencer.sequence);}
  ...
  checkpoint({record=true}={}){if(this.spatial)this.project.spatial3d=deep(this.spatial.workspace);this.persistSequence();this.project=sealProject(this.project,this.clock);if(record){this.history=this.history.slice(0,this.history_index+1);this.history.push(deep(this.project));this.history_index=this.history.length-1;this.resetRuntimeTimeline();}return this.inspect();}
  ...
  setAudioCueBinding({cueId,assetId,fileRole=null,bindingId=null}={}){
    const cue=String(cueId??'').trim(),id=String(assetId??'').trim(),record=this.project.assets.registry[id];
    if(!cue)throw new StudioError('AUDIO_TARGET_CUE_REQUIRED');
    if(!record)throw new StudioError('AUDIO_TARGET_ASSET_NOT_FOUND',id);
    const file=(record.files??[]).find(item=>fileRole?item.role===fileRole:String(item.mime??'').startsWith('audio/'));
    if(!file)throw new StudioError('AUDIO_TARGET_AUDIO_FILE_NOT_FOUND',fileRole??id);
    if(!file.sha256)throw new StudioError('AUDIO_TARGET_AUDIO_HASH_MISSING',file.role);
    const existing=this.project.audio_target?.bindings??[];
    const bindings=[...existing.filter(item=>item.cue_id!==cue),{binding_id:String(bindingId??('audio-binding:'+safeId(cue))),cue_id:cue,asset_id:id,file_role:String(file.role),asset_sha256:String(file.sha256??'').toLowerCase()}];
    this.project.audio_target=createAudioTargetProfile({bindings,fallback_policy:this.project.audio_target?.fallback_policy??'blocked'});
    this.record('audio.cue-bound',{cue_id:cue,asset_id:id,file_role:file.role,asset_sha256:file.sha256,profile_root:this.project.audio_target.profile_root});
    return this.checkpoint();
  }
  setAudioTargetSpatialPolicy({listenerId,positionScale=1000,occlusionMode='lowpass'}={}){
    if(!this.project.audio_target)throw new StudioError('AUDIO_TARGET_PROFILE_REQUIRED');
    const listener=String(listenerId??'').trim();
    if(!listener)throw new StudioError('AUDIO_TARGET_SPATIAL_LISTENER_REQUIRED');
    this.project.audio_target=createAudioTargetProfile({
      bindings:this.project.audio_target.bindings??[],
      fallback_policy:this.project.audio_target.fallback_policy??'blocked',
      spatial_policy:{listener_id:listener,position_scale:positionScale,occlusion_mode:occlusionMode}
    });
    this.record('audio.spatial-policy-set',{listener_id:listener,position_scale:positionScale,occlusion_mode:occlusionMode,profile_root:this.project.audio_target.profile_root});
    return this.checkpoint();
  }
  assetDatabasePlan(options={}){const db=createAssetDatabase(this.project,{cacheDir:options.cacheDir??path.join(process.cwd(),'output','asset-cache'),profiles:options.profiles??['runtime']});const plan=db.plan(options);this.record('asset.database-plan',{plan_root:plan.plan_root,items:plan.items.length});return plan;}
  assetDatabaseSync(options={}){const db=createAssetDatabase(this.project,{cacheDir:options.cacheDir??path.join(process.cwd(),'output','asset-cache'),profiles:options.profiles??['runtime']});const result=db.sync(options);this.project=ensureUIInputProject(result.project);this.record('asset.database-sync',{plan_root:result.plan.plan_root,sync_root:result.sync_receipt.sync_root,summary:result.summary});const inspection=this.checkpoint();return{...inspection,asset_database_plan:result.plan,asset_database_sync:result.sync_receipt,asset_database_summary:result.summary,asset_database_cache:result.cache_index};}
  async assetStreaming(options={}){const {request={},...streamOptions}=options,result=await streamSpatialAssets(this.project,request,streamOptions);this.record('asset.runtime-stream',{catalog_root:result.catalogRoot,receipt_root:result.receipt.receiptRoot,ready:result.receipt.readyAssetIds.length,failed:result.receipt.failedAssetIds.length,blocked:result.receipt.blockedAssetIds.length});return{...this.inspect(),asset_streaming_catalog_root:result.catalogRoot,asset_streaming_resolution:result.resolution,asset_streaming_receipt:result.receipt,asset_streaming_inspection:result.inspection};}
  ...
  sequenceSeek(time){const frame=this.sequencer.seek(time);this.record('sequence.seek',{time:frame.time,frame_root:frame.frame_root});return{...this.inspect(),sequence_frame:frame};}
  sequenceStep(frames=1){const frame=this.sequencer.step(frames);this.record('sequence.step',{frames,time:frame.time,frame_root:frame.frame_root});return{...this.inspect(),sequence_frame:frame};}
  sequencePlay(frames=1){const framesOut=this.sequencer.play({frames});const frame=framesOut.at(-1)??this.sequencer.last_frame;this.record('sequence.played',{frames:framesOut.length,time:frame?.time??this.sequencer.time,frame_root:frame?.frame_root??null});return{...this.inspect(),sequence_frames:framesOut,sequence_frame:frame};}
  sequenceAddClip({trackId,clip}={}){if(!trackId)throw new StudioError('SEQUENCE_TRACK_REQUIRED');const sequence=this.sequencer.addClip(trackId,clip??{});this.record('sequence.clip-added',{track_id:trackId,clip_id:clip?.clip_id??null,sequence_root:sequence.sequence_root});return this.checkpoint();}
  sequenceSnapshot(label='snapshot'){const snapshot=this.sequencer.snapshot(label);this.record('sequence.snapshot-created',{label,snapshot_root:snapshot.snapshot_root});return{...this.inspect(),sequence_snapshot:snapshot};}
  sequenceRestore(label){const restored=this.sequencer.restore(label);this.record('sequence.snapshot-restored',{label,session_root:restored.session_root});return this.checkpoint();}
  ...
  undo(){if(this.history_index<=0)return this.inspect();this.history_index--;this.project=deep(this.history[this.history_index]);const p=this.project.behavior.programs[this.project.behavior.active_program_id];this.behavior.replaceProgram(p,{preserveState:true,recordHistory:false});this.spatial=new SpatialStudioSession(this.project.spatial3d);this.sequencer=new SequencerSession(this.project.sequencer??createDefaultSequence(this.project));this.resetRuntimeTimeline();this.record('editor.undo');return this.inspect();}
  redo(){if(this.history_index>=this.history.length-1)return this.inspect();this.history_index++;this.project=deep(this.history[this.history_index]);const p=this.project.behavior.programs[this.project.behavior.active_program_id];this.behavior.replaceProgram(p,{preserveState:true,recordHistory:false});this.spatial=new SpatialStudioSession(this.project.spatial3d);this.sequencer=new SequencerSession(this.project.sequencer??createDefaultSequence(this.project));this.resetRuntimeTimeline();this.record('editor.redo');return this.inspect();}
  ...
  exportArtifacts(){
    const behavior=this.behavior.exportArtifacts();const player=createSceneProjection(this.project,this.behavior.inspect(),{observer:'player',navigationPositions:this.navigation.positions}),debuggerView=createSceneProjection(this.project,this.behavior.inspect(),{observer:'debugger',navigationPositions:this.navigation.positions});
     const gpu=this.compileGPUFrame({serialized:true});const spatial=this.spatial.exportArtifacts();const runtimeTimeline=this.timelineView(),runtimeReplay=this.replayRuntime({verify:true}),networkCompilation=this.project.network?this.networkCompile():null;
    ...
    const uiLayout=this.compileUILayout(),uiInputManifest=compileUIInputManifest({projectRoot:this.project.project_root,tree:this.activeUITree(),profile:this.activeInputProfile(),layout:uiLayout});
     ...
     const buildFiles=['project.json','scene-player.json','scene-debugger.json','behavior.json','assets.json','asset-database.json','asset-continuity-ledger.json','live-update.manifest.json','gateway.runtime.json','runtime-timeline.json','runtime-replay.json','sequence.json',...(this.project.audio_target?['audio-target.json']:[]),'gpu-viewport.manifest.json','gpu-frame-summary.json','gpu-frame-plan.json','tilemap-navigation.manifest.json','ui-input.manifest.json','spatial-workspace.json','spatial-world.json','spatial-snapshot.json','spatial-frame-plan.json','spatial-runtime.manifest.json','web-preview.html'];if(networkCompilation)buildFiles.push('network-world-compilation.json');
     const build=seal({format:'reality-studio.web-build-plan.v1.2',project_root:this.project.project_root,entry_scene_id:this.project.active_scene_id,files:buildFiles,targets:deep(this.project.build.targets),gpu:{preferred_backend:'webgpu',fallback_backend:'canvas2d',frame_plan_root:gpu.summary.frame_plan_root},ui_input:{manifest_root:uiInputManifest.manifest_root},audio_target:this.project.audio_target?{profile_root:this.project.audio_target.profile_root,binding_count:this.project.audio_target.bindings.length,spatial_policy:this.project.audio_target.spatial_policy?deep(this.project.audio_target.spatial_policy):null}:null,network:networkCompilation?{compilation_root:networkCompilation.compilation_root,world_config_root:networkCompilation.world_config_root,player_slots:networkCompilation.counts.player_slots}:null},'build_root');
     return{project:this.project,scene_player:player,scene_debugger:debuggerView,behavior_program:this.behavior.program,behavior_runtime:behavior,runtime_timeline:runtimeTimeline,runtime_replay:runtimeReplay,sequence:this.sequenceView(),audio_target:this.project.audio_target??null,asset_manifest:assetManifest,asset_database:assetDatabase,asset_continuity_ledger:assetLedger,live_update_manifest:liveUpdateManifest,gateway_manifest:gateway,build_plan:build,gpu_viewport_manifest:gpu.manifest,gpu_frame_summary:gpu.summary,gpu_frame_plan:gpu.frame,tilemap_navigation_manifest:tilemapManifest,ui_input_manifest:uiInputManifest,ui_layout:uiLayout,input_profile:this.activeInputProfile(),network_world_compilation:networkCompilation,...spatial,editor_events:deep(this.events)};
  }
  inspect(){
    ...
    return{
      ...
      behavior:b,timeline,runtime_timeline:timeline,sequence:this.sequenceView(),audio_target:deep(this.project.audio_target??null),live_update:this.liveUpdateManifest(),
