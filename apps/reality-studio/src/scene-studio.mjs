...
export class UnifiedManufacturingSession{
  ...
  exportArtifacts(){
    ...
    const uiLayout=this.compileUILayout(),uiInputManifest=compileUIInputManifest({projectRoot:this.project.project_root,tree:this.activeUITree(),profile:this.activeInputProfile(),layout:uiLayout});
     ...
     return{project:this.project,scene_player:player,scene_debugger:debuggerView,behavior_program:this.behavior.program,behavior_runtime:behavior,runtime_timeline:runtimeTimeline,runtime_replay:runtimeReplay,asset_manifest:assetManifest,asset_database:assetDatabase,asset_continuity_ledger:assetLedger,live_update_manifest:liveUpdateManifest,gateway_manifest:gateway,build_plan:build,gpu_viewport_manifest:gpu.manifest,gpu_frame_summary:gpu.summary,gpu_frame_plan:gpu.frame,tilemap_navigation_manifest:tilemapManifest,ui_input_manifest:uiInputManifest,ui_layout:uiLayout,input_profile:this.activeInputProfile(),network_world_compilation:networkCompilation,...spatial,editor_events:deep(this.events)};
  }
  inspect(){
