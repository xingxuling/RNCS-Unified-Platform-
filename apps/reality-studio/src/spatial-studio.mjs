...
} from '@taowind/reality-engine-session/spatial-replay';
...
export class SpatialStudioSession{
  ...
  createReplayBundle({ticks=1,commands=[],checkpointEvery=1,branchId=null,metadata={}}={}){const e=this.workspace.editor,position=e.camera.position_milli,rotation=e.camera.rotation_mdeg,startTick=this.lastSnapshot.tick,normalized=commands.map((command,index)=>({...deep(command),id:command.id??`studio-replay-command:${startTick+1}:${index+1}`,tick:command.tick??startTick+1})),bundle=createSpatialReplayBundle({initialSnapshot:this.lastSnapshot,commands:normalized,ticks,checkpointEvery,branchId:branchId??`branch:studio:${this.config.worldId}:${startTick}`,projectionOptions:{width:e.viewport.width,height:e.viewport.height,qualityTier:e.viewport.quality_tier,cameraPosition:[position.x/1000,position.y/1000,position.z/1000],cameraRotationDeg:[rotation.x/1000,rotation.y/1000,rotation.z/1000],includeContacts:e.show_contacts,includeSensoryEvents:e.show_sensory},metadata:{workspace_root:this.workspace.workspace_root,world_id:this.config.worldId,source_state_root:this.lastSnapshot.stateRoot,...deep(metadata)}});this.record('spatial.replay-bundle-created',{bundle_root:bundle.bundle_root,branch_id:bundle.branch_id,checkpoint_count:bundle.checkpoints.length});return bundle}
  createReplayBranch(bundle,options={}){const branch=createSpatialReplayBranch(bundle,options);this.record('spatial.replay-branch-created',{bundle_root:branch.bundle_root,parent_bundle_root:branch.parent_bundle_root,branch_id:branch.branch_id});return branch}
  compareReplayBranches(left,right){const comparison=compareSpatialReplayBranches(left,right);this.record('spatial.replay-branches-compared',{comparison_root:comparison.comparison_root,same_state:comparison.same_state,same_frame:comparison.same_frame});return comparison}
