...
export function normalizeSequence(raw={}){
  ...
  const seq={format:'reality-studio.sequence.v1.6',sequence_id:raw.sequence_id??`sequence:${randomUUID()}`,title:raw.title??'Untitled Sequence',fps:Number(raw.fps??30),duration:Number(raw.duration??Math.max(0,...tracks.flatMap(t=>t.clips.map(c=>c.start+c.duration)))),tracks};
...
export class SequencerSession{
  constructor(sequence){this.sequence=normalizeSequence(sequence);this.time=0;this.playing=false;this.history=[];this.snapshots=new Map();}
  seek(time){const previous=this.time;this.time=Math.min(this.sequence.duration,Math.max(0,Number(time)));const frame=evaluateSequence(this.sequence,this.time,{previousTime:previous});this.history.push(frame);return frame;}
  ...
  inspect(){return{format:'reality-studio.sequencer-session.v1.6',time:this.time,playing:this.playing,sequence:clone(this.sequence),history_length:this.history.length,session_root:hash({time:this.time,sequence_root:this.sequence.sequence_root,history:this.history.map(x=>x.frame_root)})};}
}
