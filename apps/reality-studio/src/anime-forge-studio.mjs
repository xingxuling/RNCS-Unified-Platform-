...
export class AnimeForgeSession{
  ...
  visualQuality(){
    ...
    const provider=createLocalDiffusionProviderManifest(),model=createVisualModelManifest({model_id:'not-configured',model_revision:'not-configured',status:'missing',source:{kind:'studio-environment-audit',ref:'no-local-visual-model'},license:{status:'missing',spdx:null,source:null},runtime:{framework:'diffusers',version:null,device:'cpu-only'}});
    const reference=createCharacterReferencePack({character_id:characterId,identity_root:binding?.identity_root??null,genome_root:binding?.genome_root??null,appearance_root:binding?.appearance_root??null,lineage_root:binding?.family_root??null,references:[],fixed_palette:binding?.palette??{},facial_features:binding?.face_features??{},hair_structure:{family:binding?.appearance?.hair_family??null},costume_structure:{family:binding?.appearance?.costume_family??null,variant:binding?.appearance?.costume_variant??null},forbidden_variation:['identity-root change','palette drift','hair-family change','costume-family change'],source:{kind:'ragf-character-genome-binding',ref:binding?.binding_root??'missing'},license:{status:binding?.provider?.license??'pending',spdx:binding?.provider?.license??null,source:'RAGF binding'},status:'blocked',human_character_acceptance:'pending'});
    const conditions=CONDITION_TYPES.map(type=>({type,status:'missing',root:rootHash({type,shot_intent_root:shotIntentRoot,source:'provider-blocked'}),path:null})),condition=createVisualConditionPack({episode_intent_root:this.production.episode_intent_root,shot_intent_root:shotIntentRoot,character_identity_root:binding?.identity_root??null,reference_pack_root:reference.reference_pack_root,conditions,status:'blocked'});
    const candidate=createVisualCandidateBranch({candidate_id:`visual-candidate:${this.production.production_id}:${cut.cut_id}`,candidate_status:'blocked',episode_intent_root:this.production.episode_intent_root,shot_intent_root:shotIntentRoot,character_identity_root:binding?.identity_root??null,character_reference_pack_root:reference.reference_pack_root,condition_pack_root:condition.condition_pack_root,provider_manifest:provider,model_id:model.model_id,model_revision:model.model_revision,model_license:model.license,model_license_root:model.license_root,model_manifest_root:model.model_manifest_root,seed:null,sampler:null,scheduler:null,environment:{provider_executable_env:'VISUAL_PROVIDER_EXECUTABLE',provider_args_env:'VISUAL_PROVIDER_ARGS_JSON',runtime:'local-audit'},gpu:{status:'unverified',name:null,vram_mb:null},errors:['MODEL_MISSING','REFERENCE_PACK_INVALID','CONDITION_PACK_INVALID'],human_visual_acceptance:'pending'});
    return{format:'reality-studio.anime-visual-quality-workspace.v0.1',version:'0.1.0-alpha.1',phase:'phase-5-visual-body-replacement',episode:{production_id:this.production.production_id,production_root:this.production.production_root,episode_intent_root:this.production.episode_intent_root,playable:Boolean(this.last_media?.ok),authority:'Episode Production IR'},shot:{cut_ref:canonicalCutRef(cut),cut_id:cut.cut_id,duration_seconds:cut.duration,fps:cut.fps,resolution:cut.resolution,shot_intent_root:shotIntentRoot},provider:{provider_id:provider.provider_id,provider_version:provider.provider_version,status:'blocked',roles:provider.roles,manifest_root:provider.manifest_root,media:provider.media},model:{model_id:model.model_id,model_revision:model.model_revision,status:model.status,manifest_root:model.model_manifest_root,license_root:model.license_root},reference_pack:{status:reference.status,root:reference.reference_pack_root,identity_root:reference.identity_root,views:reference.reference_views,human_character_acceptance:reference.human_character_acceptance},condition_pack:{status:condition.status,root:condition.condition_pack_root,missing:CONDITION_TYPES},candidate:{status:candidate.candidate_status,root:candidate.candidate_branch_root,human_visual_acceptance:candidate.human_visual_acceptance,commit_available:false,errors:candidate.errors},worst_problem:'MODEL_MISSING',failure_reason:'MODEL_MISSING',visual_uplift_vs_phase4:'blocked:not-compared',human_visual_acceptance:'pending',final_media:{playable:Boolean(this.last_media?.ok),mp4:this.last_media?.summary?.mp4??null},next_human_judgement:'Provide a real visual model/provider/GPU run, then compare the ordinary viewer view before any selection or commit.',authority:{episode_write:false,character_identity_write:false,automatic_score_can_commit:false,commit_requires_authorized_selection:true},boundary:'Phase 5 visual replacement is blocked; Phase 4 media remains a negative baseline only'};
  }
  visualCandidateReview(workspace=this.visualQuality()){const actionIds=['reject','revision-requested','select','commit-preview','rollback'];return{status:workspace.candidate?.status??'blocked',view:'side-by-side',candidates:[],frame_step:{enabled:false,current_frame:null,total_frames:0,reason:'no real candidate frame sequence is available'},contact_sheet:null,comparison:workspace.visual_uplift_vs_phase4??'blocked:not-compared',actions:Object.fromEntries(actionIds.map(id=>[id,{enabled:false,reason:'VISUAL_CANDIDATE_BLOCKED'}])),authority:{episode_write:false,character_identity_write:false,automatic_selection:false,automatic_commit:false}}}
  visualCandidateAction(action){const allowed=['reject','revision-requested','select','commit-preview','rollback'];if(!allowed.includes(action))throw Object.assign(new Error(`VISUAL_CANDIDATE_ACTION_UNKNOWN:${action}`),{code:'VISUAL_CANDIDATE_ACTION_UNKNOWN'});const workspace=this.visualQuality();return{ok:false,status:'blocked',code:'VISUAL_CANDIDATE_BLOCKED',action,reason:'No real external visual candidate is available; no Studio action can create or commit pixels.',candidate:workspace.candidate,review:this.visualCandidateReview(workspace),authority:{episode_write:false,character_identity_write:false,automatic_selection:false,automatic_commit:false}}}
