...
repo = root.parents[1]
hlod_output = repo / 'packages' / 'world' / 'visual-state-runtime' / 'outputs' / 'spatial-hlod'
...
try:
    ...
    with sync_playwright() as playwright:
        ...
        screenshot = artifacts / 'REALITY_STUDIO_HLOD_WEBGPU.png'
        page.locator('#hlodCanvas').screenshot(path=str(screenshot))
        shader_messages = [
        ...
        assert compiled['far']['drawNodeIds'] == ['proxy:town'], compiled
        ...
        result = {
            'format': 'reality-studio.browser-hlod-webgpu-test.v1.0',
            'ready': True,
            'health': {'studio_version': health.get('studio_version'), 'vsr_version': health.get('vsr_version')},
            'roots': {'nearFrameRoot': near_plan['frameRoot'], 'farFrameRoot': far_plan['frameRoot'], 'nearHlodRoot': near_plan['hlod']['root'], 'farHlodRoot': far_plan['hlod']['root'], 'evidenceRoot': evidence['evidenceRoot']},
            'compiled': compiled,
            'receipts': execution,
            'transition_frame_roots_match': execution['near']['frameRoot'] == near_plan['frameRoot'] and execution['far']['frameRoot'] == far_plan['frameRoot'],
            'png': str(screenshot),
        ...
        (artifacts / 'REALITY_STUDIO_HLOD_WEBGPU.json').write_text(json.dumps(result, ensure_ascii=False, indent=2) + '\n', encoding='utf-8')
