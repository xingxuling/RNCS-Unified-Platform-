...
test('Anime Forge Studio runs the Phase 1 chain through evidence and replay',async()=>{
  ...
  try{
    ...
    const session=started.animeForgeSessions.get(sessionId);assert.equal(session.production.cut.voice_track.filter(item=>item.active).length,1);assert.ok(session.production.cut.voice_track.filter(item=>item.active===false).length>=1);assert.deepEqual(session.inspect().asset_family_roots,beforeReplacement.value.asset_family_roots);
