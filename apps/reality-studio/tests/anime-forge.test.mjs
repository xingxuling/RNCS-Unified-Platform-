...
test('Anime Forge Studio preserves the Phase 1 Cut and runs the Phase 3 editorial chain',async()=>{
  ...
  try{
    ...
    const sheet=await post(started.url,'/api/anime-forge/xsheet',{session_id:sessionId});assert.equal(sheet.value.ok,true);assert.equal(sheet.value.xsheet.frames.length,120);assert.ok(sheet.value.xsheet.ragf_motion_track_root);assert.equal(sheet.value.xsheet.frames[0].ragf_motion_state.source_track_root,sheet.value.xsheet.ragf_motion_track_root);assert.equal(sheet.value.xsheet.frames[0].dialogue_state,null);assert.match(sheet.value.xsheet.frames[35].secondary_motion_state.hair_override,/^motion:hair:/);
    const candidate=await post(started.url,'/api/anime-forge/control-plane',{session_id:sessionId});assert.equal(candidate.value.ok,true,`control-plane response ${candidate.status}: ${JSON.stringify(candidate.value)}`);assert.equal(candidate.value.status,'candidate');assert.equal(candidate.value.commit_permitted,false);assert.ok(candidate.value.candidate_branch.branch_id);
...
test('Anime Forge Studio exposes a blocked Phase 5 visual quality workspace without commit authority',async()=>{
  ...
  try{
    ...
    assert.equal(route.value.candidate_review.status,'blocked');assert.equal(route.value.candidate_review.frame_step.enabled,false);assert.equal(route.value.candidate_review.actions.select.enabled,false);
    const action=await post(started.url,'/api/anime-forge/visual-candidate-action',{session_id:compiled.value.session_id,action:'select'});assert.equal(action.status,200);assert.equal(action.value.ok,false);assert.equal(action.value.code,'VISUAL_CANDIDATE_BLOCKED');assert.equal(action.value.authority.automatic_commit,false);
