...
test('server exposes project and behavior native endpoints',async()=>{
  ...
  try{
    ...
    let us=await post(url,'/api/unified/session/new',{project:unified});assert.equal(us.assets.count,4);assert.equal(us.sequence.version,'1.7.0-alpha.1');assert.ok(us.sequence.sequence.tracks.some(track=>track.type==='camera'));
    us=await post(url,'/api/unified/session/command',{session_id:us.session_id,command:'sequence-add-clip',track_id:'track:audio',clip:{clip_id:'audio:server',start:1,duration:0.5,payload:{cue:'server'}}});assert.ok(us.sequence.sequence.tracks.find(track=>track.track_id==='track:audio').clips.some(clip=>clip.clip_id==='audio:server'));
    let sequence=await post(url,'/api/unified/session/command',{session_id:us.session_id,command:'sequence-seek',time:1.1});assert.equal(sequence.sequence_frame.time,1.1);assert.ok(sequence.sequence_frame.presentation_state.some(item=>item.clip_id==='audio:server'));
    const sequenceSnapshot=await post(url,'/api/unified/session/command',{session_id:us.session_id,command:'sequence-snapshot',label:'server-sequence'});assert.equal(sequenceSnapshot.sequence_snapshot.label,'server-sequence');assert.ok(sequenceSnapshot.sequence_snapshot.snapshot_root);
    sequence=await post(url,'/api/unified/session/command',{session_id:us.session_id,command:'sequence-step',frames:1});assert.ok(Math.abs(sequence.sequence_frame.time-(1.1+1/sequence.sequence.sequence.fps))<1e-12);
    sequence=await post(url,'/api/unified/session/command',{session_id:us.session_id,command:'sequence-restore',label:'server-sequence'});assert.equal(sequence.sequence.time,1.1);assert.equal(sequence.sequence.sequence.tracks.find(track=>track.track_id==='track:audio').clips.some(clip=>clip.clip_id==='audio:server'),true);
    ...
    const streamed=await post(url,'/api/unified/session/command',{session_id:us.session_id,command:'asset-stream',cache_dir:path.join(dir,'asset-cache'),requested_asset_ids:[us.project_full.assets.order.at(-1)]});assert.ok(streamed.asset_streaming_receipt.receiptRoot);assert.deepEqual(streamed.asset_streaming_receipt.failedAssetIds,[]);assert.equal(streamed.asset_streaming_receipt.readyAssetIds.length,1);
