import { FREE_INPUT_DISCLAIMER } from "@/constants/free-input/freeExampleInputs";

export function FreeSafetyNote({ notes }: { notes: string[] }) {
  return (
    <div className="rounded-md border border-amber-500/30 bg-amber-500/5 p-3 text-xs space-y-1.5">
      ...
      {notes.length === 0 ? (
        <div className="text-muted-foreground">本次输入未触发额外安全提示。</div>
      ) : (
