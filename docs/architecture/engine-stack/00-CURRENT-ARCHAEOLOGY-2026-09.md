...
固定 clip、VSR layers 和 VSR graph 的 animation target lowering 已完成 candidate 验证并分别封存；它们不再是“Build 完全没有既有 VSR 动画执行接缝”的未实现项，但 Studio authored Sequence lowering、事件驱动 transition 仍属于后续有边界的候选，不应从 fixed-tick 证据外推。音频显式 target binding 已经通过既有 VSR asset residency 接缝完成 Chromium 与 Android WebView candidate；这不等于 voice/mixer/continuous-stream policy 已实现。
