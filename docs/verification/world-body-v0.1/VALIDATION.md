...
The root scripts now delegate to the existing RCL native-boundary verifier and build RSR (whose prebuild prepares VSR) before gateway tests. The exact local gates then passed: gateway `26/26`, and the Windows boundary returned `NATIVE_WINDOWS_VERIFIED`. This verifies the checked native executable and smoke path; it still does not claim a local native source rebuild.
