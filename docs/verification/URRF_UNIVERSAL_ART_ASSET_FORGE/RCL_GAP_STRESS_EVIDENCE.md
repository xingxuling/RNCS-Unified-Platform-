...
| EVIDENCE | CANDIDATE | Report, Genome, Acceptance, GLB inspection, Evidence Ledger, multi-asset batch, assembly, dependency-complete VSR projection/materialization, CPU import report, and an explicit `INCOMPLETE` evidence-bundle packet are committed; no real approvals are present. |
