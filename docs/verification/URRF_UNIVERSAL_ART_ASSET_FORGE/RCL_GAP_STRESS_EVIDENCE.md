...
- a canonical Provider execution-receipt primitive that binds one request to Job/Result/Failure, candidate/court, materialization, file inspection, acceptance and ledger roots, and records replay as unrun until a real Provider is rerun.
...
The negative cases are part of the contract: a Provider result, a generated GLB, a local browser/runtime receipt, an invalid LOD sequence, Provider provenance/license/`quality_tier` metadata, or Provider `art_direction`/`human_review` declarations are not by themselves AAA acceptance or RNCS authority. A malformed materialized GLB, an unbound PBR material, an unbound/non-external provenance/license, review, or quality receipt, a metric-mismatched quality proof, duplicate/non-reducing LOD geometry, stale or tampered assembly file bytes, unsafe assembly paths, invalid LOD/placement selections, an unregistered asset kind, a Provider-preflight route/source/health/license tamper, a Provider execution receipt request/output/candidate tamper, a missing external replay, a tampered VSR metadata root, a tampered evidence-bundle root, or a tampered cross-asset index cannot be rescued by Provider `PASS` declarations.
...
- `node --test packages/world/large-world-runtime/tests/universal-art-asset-forge.test.mjs`: `21/21 PASS`, including unknown-asset-kind rejection, all-profile route-boundary, Provider preflight replay, injected-runtime classification, and fail-closed tamper coverage.
...
- `node --test tests/large-world-universal-art-asset-forge.integration.test.mjs`: `5/5 PASS`, including the persisted nine-profile coverage matrix, Provider preflight packet, and two-profile holdout packet.
...
| ROBUST | CANDIDATE | Missing runtime, missing profile, silent profile fallback, unregistered asset kinds, Provider preflight route/health/license tamper, Provider execution-root tamper/missing replay, and weakened evidence fail closed. |
...
7. Rerun the same bound Provider request on the real external runtime and compare independent output roots; the current execution receipt records `replay.status=NOT_RUN` and deliberately does not claim determinism.
