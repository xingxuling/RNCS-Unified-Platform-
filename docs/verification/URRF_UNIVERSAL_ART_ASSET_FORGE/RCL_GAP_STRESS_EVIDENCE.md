...
- `node --test packages/world/large-world-runtime/tests/universal-art-asset-forge.test.mjs`: `22/22 PASS`, including unknown-asset-kind rejection, all-profile route-boundary, Provider preflight replay, Provider execution replay root comparison, injected-runtime classification, nondeterminism rejection, and fail-closed tamper coverage.
- `npm run test:gltf --workspace @taowind/visual-state-runtime`: `22/22 PASS`, including the external PBR binding adapter.
- `npm test --workspace @taowind/large-world-runtime`: `60/60 PASS`.
- `npm run test:large-world-universal-art-asset-forge`: package `60/60 PASS`, integration `5/5 PASS`.
...
- `node --test tests/large-world-universal-art-asset-forge.integration.test.mjs`: `5/5 PASS`, including the persisted nine-profile coverage matrix, Provider preflight packet, Provider execution/replay packets, and two-profile holdout packet.
...
- Schema validation: `PASS` for 20 URRF schemas (37 large-world schema definitions total) covering the Forge, persisted forge report, GLB inspection, independent provenance/license, review-receipt, quality-proof, evidence-bundle, batch, holdout, all-profile coverage, Provider preflight, Provider execution receipt, Provider replay report, assembly, VSR projection, VSR payload materialization, VSR import-execution, VSR WebGPU report, VSR browser-receipt, and the sustained WebGPU performance envelope; 17 persisted URRF packets validate with Draft 2020-12 and their roots reverify.
- Persisted evidence audit: `PASS` for the file-inspection, incomplete evidence-bundle, batch, holdout, all-profile coverage, Provider preflight, Provider execution receipt, Provider replay report, replay execution receipt, assembly, VSR projection, VSR payload materialization, VSR import-execution, aggregate scene/CPU reference, VSR WebGPU report, Chromium browser-receipt, and sustained-performance packets; profile mode/source boundaries, Provider manifest health/license/readiness boundaries, request/Job/Result/Failure and materialization roots, replay request/provider/mode bindings and output-root equality, holdout batch/root isolation, mesh/PBR payload roots, streamer receipt root, import roots, CPU frame roots, browser GPU receipt root, performance root, and CPU/GPU frame-root equality reverify, while the incomplete bundle, profile-source tamper, Provider-preflight tamper, Provider-execution tamper, replay mismatch, and holdout tamper cases remain fail-closed.
...
| LOWER | EVIDENCED | RAGF/VSR workspace, Provider Adapter, execution receipt, multi-asset assembly, dependency-complete PBR catalog, and VSR projection paths are connected. |
...
| ROBUST | CANDIDATE | Missing runtime, missing profile, silent profile fallback, unregistered asset kinds, Provider preflight route/health/license tamper, Provider execution-root tamper, replay request/output-root mismatch, missing external replay, and weakened evidence fail closed. |
