...
- Schema validation: `PASS` for 17 schemas covering the Forge, persisted forge report, GLB inspection, independent provenance/license, review-receipt, quality-proof, evidence-bundle, batch, holdout, all-profile coverage, assembly, VSR projection, VSR payload materialization, VSR import-execution, VSR WebGPU report, VSR browser-receipt, and the sustained WebGPU performance envelope; generated envelopes validate with Draft 2020-12 and their roots reverify.
- Persisted evidence audit: `PASS` for the file-inspection, incomplete evidence-bundle, batch, holdout, all-profile coverage, assembly, VSR projection, VSR payload materialization, VSR import-execution, aggregate scene/CPU reference, VSR WebGPU report, Chromium browser-receipt, and sustained-performance packets; profile mode/source boundaries, holdout batch/root isolation, mesh/PBR payload roots, streamer receipt root, import roots, CPU frame roots, browser GPU receipt root, performance root, and CPU/GPU frame-root equality reverify, while the incomplete bundle, profile-source tamper, and holdout tamper cases remain fail-closed.
...
| COMPILE | EVIDENCED | RAGF intent/genome, provider resolution, and profile mode classification compile. |
