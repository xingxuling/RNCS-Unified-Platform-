...
- Schema validation: `PASS` for 9 schemas covering the Forge, persisted GLB inspection, independent provenance/license, review-receipt, quality-proof, evidence-bundle, batch, assembly, and VSR projection envelopes; generated envelopes validate with Draft 2020-12 and their roots reverify.
- Persisted evidence audit: `PASS` for the file-inspection, incomplete evidence-bundle, batch, assembly, and VSR projection packets; batch/assembly/projection roots reverify and the incomplete bundle remains fail-closed.
...
| LOWER | EVIDENCED | RAGF/VSR workspace, Provider Adapter, multi-asset assembly, and VSR projection-reference paths are connected. |
| EXECUTE | EVIDENCED_CANDIDATE | Real local RAGF files are rehashed at assembly binding; GLB inspection and injected-provider job execution remain candidate evidence. |
...
| EVIDENCE | CANDIDATE | Report, Genome, Acceptance, GLB inspection, Evidence Ledger, multi-asset batch, assembly, VSR projection, and an explicit `INCOMPLETE` evidence-bundle packet are committed; no real approvals are present. |
...
6. Execute the produced VSR projection through the target importer/GPU/device path and independently verify material residency, scene correctness, frame budget, and production delivery; the current projection is only a rooted local reference envelope.
