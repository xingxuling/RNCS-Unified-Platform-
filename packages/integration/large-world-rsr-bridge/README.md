...
The RSR runtime remains the physical owner. The adapter is deliberately not a dependency of Large World Runtime and does not implement collision, ray casts, terrain streaming, or a second terrain generator. RSR now exposes a bounded authored-surface ray, bounded authored-triangle sphere sweep, bounded upright-capsule-vs-triangle sweep, and a bounded sampled contact-manifold path for the lowered fixture; the adapter only reaches those existing runtime paths. The current RSR heightfield implementation still layers at most four footprint support points over a single-support/AABB penetration candidate, not a continuous terrain contact manifold, dynamic terrain path, or production device path.
