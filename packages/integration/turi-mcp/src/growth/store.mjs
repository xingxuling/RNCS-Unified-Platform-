...
export class GrowthStore {
  ...
  recordExperience(input = {}) {
    ...
    const experience = {
      ...
      successfulSteps: clone(input.successfulSteps ?? []), failedSteps: clone(input.failedSteps ?? []), rejectedAlternatives: clone(input.rejectedAlternatives ?? []), residuals: clone(input.residuals ?? []), userFeedback: clone(input.userFeedback ?? []), assistance: clone(input.assistance ?? null), evidenceReceipts, artifactRefs: Array.isArray(input.artifactRefs) ? input.artifactRefs.map(String) : [],
