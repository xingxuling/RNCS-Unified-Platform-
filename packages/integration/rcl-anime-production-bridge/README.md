...
Compiled RAGF Anime families are automatically resolved into per-Cut motion tracks by `src/ragf-motion-runtime.mjs`. Matching uses the derived `family_root` first, then asset and actor identity; invalid or ambiguous tracks fail closed. The built-in loopable motion contract uses an explicit `loop` policy with a declared cycle period, so long Cuts continue hair, coat, breathing and blink coverage on the destination timebase. The cycle terminal frame is checked against the first frame through a shared `motion_quality_root`, and the bridge report records its per-channel seam result. `hold-last` remains available as an explicit compatibility policy for legacy tracks. The binding carries each Cut's RSR director overrides, records source-cycle coverage and never changes Episode authoring authority. Compile and media builds write `ragf-motion-runtime-binding-report.json`; `media`, `xsheet`, `render` and `replay` reuse the same binding path. The media build also records the binding root in its deterministic roots and Evidence Ledger.
The same report now includes `motion_visual_quality_root` and `coverage.visual_state_quality`. This report is computed from the RAGF source track's RGBA8 pixel roots and frame deltas, so all derived Cuts can prove they reuse one visually verified state track. It remains experimental reference evidence, not commercial animation acceptance.
