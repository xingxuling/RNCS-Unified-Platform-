...
Compiled RAGF Anime families are automatically resolved into per-Cut motion tracks by `src/ragf-motion-runtime.mjs`. Matching uses the derived `family_root` first, then asset and actor identity; invalid or ambiguous tracks fail closed. The runtime binding uses an explicit `hold-last` policy for the short built-in motion sequence, carries each Cut's RSR director overrides, and never changes Episode authoring authority. Compile and media builds write `ragf-motion-runtime-binding-report.json`; `media`, `xsheet`, `render` and `replay` reuse the same binding path. The media build also records the binding root in its deterministic roots and Evidence Ledger.
