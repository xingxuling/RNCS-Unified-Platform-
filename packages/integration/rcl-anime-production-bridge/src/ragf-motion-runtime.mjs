import {analyzeAnimeMotionTrack,analyzeAnimeMotionVisualFrames,canonicalCutRef,listProductionCuts,rootHash,seal,validateRagfMotionTrack} from '../../../world/anime-production-runtime/src/index.mjs';
...
function motionCoverage(cut,track,policy){
  ...
  const motionQuality=analyzeAnimeMotionTrack(track),visualStateQuality=analyzeAnimeMotionVisualFrames({frames:track.frames,loopMode:track.loop_mode,loopPeriodFrames:track.loop_period_frames});
  return{expected_frame_count:expectedFrameCount,source_frame_count:Number(track.frame_count),cycle_frame_count:period,reachable_source_frame_count:reachable.size,source_cycle_coverage_ratio:period?Math.round(reachable.size/period*10000)/10000:0,cycle_coverage_complete:policy==='loop'&&reachable.size>=period,loop_iterations:iterations.size,runtime_coverage_complete:true,motion_quality_root:track.quality_contract?.quality_root??rootHash(motionQuality),motion_visual_quality_root:track.quality_contract?.visual_quality_root??rootHash(visualStateQuality),seam_quality:motionQuality.seam,visual_state_quality:visualStateQuality};
}
