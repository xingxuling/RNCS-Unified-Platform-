...
The current donor now consumes dynamic mass through the shared optional `spatial.body.mass_q` → RSR `SpatialBodySpec.massQ` lowering, the complete fixture collection through `spatial.fixtures.items` → RSR `SpatialBodySpec.fixtures[]`, and Studio character controllers through `spatial.character` → RSR character specs. Verified asset instances are lowered through the existing Reality Cell asset streamer/GLB importer, including repeated instances of one content-addressed payload; verified Network Compilation player slots are lowered into the existing observer relevance profile. Any unconfigured asset/network facets remain explicit `RCL_GAP_WB_AETHER_*` losses rather than being silently dropped. `verifyStudioWorldBodyAetherProjection()` binds the receipt to the emitted runtime roots and checks any executed asset/relevance bindings.
