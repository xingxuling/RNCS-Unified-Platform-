...
The current donor now consumes dynamic mass through the shared optional `spatial.body.mass_q` → RSR `SpatialBodySpec.massQ` lowering and Studio character controllers through `spatial.character` → RSR character specs. It still cannot consume secondary fixtures, World Body visual asset bindings, or verified Network Compilation facets. Those remaining facets are reported as `RCL_GAP_WB_AETHER_*` losses rather than being silently dropped. `verifyStudioWorldBodyAetherProjection()` binds the receipt to the emitted runtime roots.
