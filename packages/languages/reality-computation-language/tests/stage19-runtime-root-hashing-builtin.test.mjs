...
test('Stage-19 RCL-owned runtime interpreter computes transaction roots through SHA256_TEXT builtin', () => {
  const out = execFileSync('node', ['scripts/verify-rcl-selfhost-stage19.mjs'], {
  ...
  assert.equal(report.stageStatus, 'RCL_OWNED_RUNTIME_ROOT_HASHING_BUILTIN_SUBSET_VERIFIED');
  ...
  assert.equal(report.checks.decodedInterpreterCallsSha256TextBuiltin, true);
  assert.equal(report.checks.rclComputedRootsAreNotLiteralOnly, true);
  ...
  assert.equal(report.boundaries.notYetImplemented.includes('not a pure RCL SHA-256 algorithm'), true);
});
