...
const rclPath = path.join(root, 'selfhost', 'rcl-rule-lowering-loop-stage31.rcl');
const outputDir = path.join(root, 'output', 'selfhost');
const outputPath = path.join(outputDir, 'stage31-verification.json');
const interpreterArtifactPath = path.join(outputDir, 'stage31-rule-lowering-loop-interpreter.rbc');
const targetRbcPath = path.join(outputDir, 'stage31-rule-lowering-loop-target.rbc');
const referenceRbcPath = path.join(outputDir, 'stage31-rule-lowering-loop-js-reference.rbc');
...
const checks = {
  ...
  interpreterRunsInNativeVm: interpreterRun.status === 'ok'
    && state['selfhost.stage_status'] === 'STAGE31_RCL_OWNED_RULE_LOWERING_LOOP_SUBSET_VERIFIED',
  stage31HeaderCorrect: state['selfhost.stage'] === 'stage31_rcl_owned_rule_lowering_loop_subset'
    && state['selfhost.claim'] === 'rcl_lowers_directive_sequence_with_rule_lookup_to_bytecode'
    && state['selfhost.boundary'] === 'rule_lowering_loop_subset_not_complete_parser_compiler_or_runtime'
  ...
  sourceTargetCorrect: state['compiler.program'] === 'Stage31Target'
  ...
  tokenizerWorks: state['parser.token_count'] > 0
    && state['parser.program_token'] === 'Stage31Target',
  compilerParsesSource: compilerProgram.name === 'Stage31Target'
  ...
  targetRbcGenerated: targetRbc.length > 0
    && decodedTarget.program === 'Stage31Target'
  ...
  targetHasCorrectInstructionCount: decodedTarget.instructions.length === 117
    && state['target.rbc_instruction_count'] === 117,
  targetHasCorrectStringPool: decodedTarget.strings.includes('Stage31Target')
    ...
    && decodedTarget.strings.includes('rcl:stage31:score')
    && decodedTarget.strings.includes('rcl:stage31:level')
    && decodedTarget.strings.includes('rcl:stage31:certify'),
  ...
  ruleLoweringLoopEvidence: state['compiler.emergence_count'] === 3
    && state['compiler.directive_count'] === 6
    && state['compiler.rule_lowering_loop_supported'] === true
  ...
  boundaryHonest: state['selfhost.boundary'] === 'rule_lowering_loop_subset_not_complete_parser_compiler_or_runtime'
    && state['gate.rcl_owned_general_expression_parser_subset'] === true
    && state['gate.rcl_owned_rule_lowering_loop'] === true
    && state['gate.rcl_owned_runtime_complete'] === false,
};
...
const payload = {
  ...
  format: 'rcl.selfhost.stage31.verification.v1',
  ...
  boundaries: {
    implementedNow: 'RCL builds a parsed rule sequence, a directive sequence, looks rules up by directive name, and lowers six rule invocations through a recursive bytecode emission loop. The generated target RBC matches the JS compiler byte-for-byte.',
