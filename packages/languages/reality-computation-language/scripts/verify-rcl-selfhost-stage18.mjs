...
const rclPath = path.join(root, 'selfhost', 'rcl-runtime-history-root-interpreter-stage18.rcl');
const outputDir = path.join(root, 'output', 'selfhost');
const outputPath = path.join(outputDir, 'stage18-verification.json');
const interpreterArtifactPath = path.join(outputDir, 'stage18-runtime-history-root-interpreter.rbc');
const targetRbcPath = path.join(outputDir, 'stage18-runtime-history-root-target.rbc');
const jsReferenceTargetRbcPath = path.join(outputDir, 'stage18-runtime-history-root-target-js-reference.rbc');
...
const checks = {
  interpreterRunsInNativeVm: interpreterRun.status === 'ok'
    && state['selfhost.stage_status'] === 'RCL_OWNED_RUNTIME_HISTORY_ROOT_PREIMAGE_SUBSET_VERIFIED',
  ...
  boundaryHonest: state['selfhost.boundary'] === 'single_rule_transaction_history_root_preimage_subset_not_complete_rcl_runtime_or_sha256_builtin'
...
const payload = {
  ...
  format: 'rcl.selfhost.stage18.verification.v1',
  ...
  boundaries: {
    implementedNow: 'A native-running RCL artifact interprets the Stage-15 authorized transaction subset and now also emits canonical before/after state preimages plus the projection/history root values for that transaction. The emitted preimages hash to the native and JS beforeRoot/afterRoot records.',
    notYetImplemented: 'The RCL artifact owns the canonical preimage shape for this single ready/status transaction subset, but SHA-256 hashing is still verified by JS/native host code. This is not full root parity, not arbitrary canonical serialization, not complete error-path/provider parity, and not a complete RCL-owned runtime.',
