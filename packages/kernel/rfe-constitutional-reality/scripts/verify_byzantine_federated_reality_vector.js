...
import { verifyByzantineFederatedRealityResult } from '../reference-js/src/byzantine-federated-reality-v080.js';
...
const vector = levels.c10;
verifyByzantineFederatedRealityResult(vector.expected);
...
console.log(JSON.stringify({
  ...
  vectorHash,
  c10ResultHash: vector.expected.byzantineFederatedRealityResultHash,
  initialFederationRoot: vector.expected.initialFederationRoot,
  finalFederationRoot: vector.expected.finalFederationRoot,
  approvalWeight: vector.expected.certificate.approvalWeight,
  forkEffectiveWeight: vector.expected.rejectedFork.effectiveApprovalWeight,
  quarantinedWeight: vector.expected.metrics.quarantinedWeight,
  status: 'PASS',
}, null, 2));
