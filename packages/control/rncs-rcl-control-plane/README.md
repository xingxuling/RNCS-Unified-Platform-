...
对于空间世界，控制面还可从 native `Sequence` facet 声明提取受限的 `rncs.rcl-spatial-command-plan.v0.1`。它只生成带 RCL state/command root 的 candidate plan；进入 RSR 必须经过独立的 `@taowind/rcl-rsr-spatial-bridge`，当前仅覆盖 bounded `patch-heightfield`，不等于 RCL 已有一等 typed spatial primitive。
