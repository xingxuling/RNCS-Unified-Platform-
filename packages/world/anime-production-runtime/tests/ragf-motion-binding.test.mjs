...
test('RAGF runtime carries and validates visual state quality roots',()=>{const track=source().family.motion_track,result=validateRagfMotionTrack(track);assert.equal(result.valid,true);assert.equal(result.visual_quality.status,'pass');assert.equal(result.motion_visual_quality_root,track.quality_contract.visual_quality_root);const sheet=buildExposureSheet(cut({duration:2,fps:24}),{ragfMotionTrack:track,ragfMotionBindingOptions:{missing_frame_policy:'loop'}}),bound=bindRagfMotionTrackToXSheet(sheet,track,{missing_frame_policy:'loop'}),binding=validateRagfMotionBinding(bound.binding);assert.equal(binding.valid,true);assert.equal(bound.binding.motion_visual_quality_root,track.quality_contract.visual_quality_root);assert.equal(bound.binding.frame_map[7].pixel_root,track.frames[3].pixel_root);assert.equal(bound.xsheet.ragf_motion_binding.motion_visual_quality_root,track.quality_contract.visual_quality_root);});
test('RAGF runtime rejects a track with forged pixel coverage',()=>{const generated=source(),tampered=structuredClone(generated.family.motion_track);tampered.frames[7].pixel_root='0'.repeat(64);assert.equal(validateRagfMotionTrack(tampered).valid,false);assert.ok(validateRagfMotionTrack(tampered).errors.includes('RAGF_MOTION_VISUAL_QUALITY_INVALID'));});
