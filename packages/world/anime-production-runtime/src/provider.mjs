...
export function createBuiltinAnimeProviderManifests(){
  ...
  return manifests.map(manifest=>manifest.provider_id==='ragf.anime-builtin-generator'?seal({...manifest,outputs:[...manifest.outputs,'motion-loop-contract','motion-seam-quality-report','motion-visual-quality-report'],motion:{...manifest.motion,capabilities:[...manifest.motion.capabilities,'loopable-motion-sequence','seam-continuity-quality','pixel-verified-state-coverage'],binding_contract:{...manifest.motion.binding_contract,policies:['strict','hold-first','hold-last','loop'],default_policy:'loop'}},evidence:{...manifest.evidence,outputs:[...manifest.evidence.outputs,'coverage-report','loop-iterations','seam-quality','pixel-roots']},quality_boundary:'deterministic experimental Anime assets with RGBA8 pixel-verified state coverage, explicit cycle coverage and terminal seam quality; commercial source-art quality and physical simulation not proven',manifest_root:''},'manifest_root'):manifest);
}
