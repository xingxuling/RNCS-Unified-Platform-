...
export function createAnimeProviderManifest(input={}){
  ...
  const manifest={
    ...
    motion:{capabilities:array(input.motion?.capabilities),xsheet_track_required:Boolean(input.motion?.xsheet_track_required)},
...
export function createBuiltinAnimeProviderManifests(){
  ...
  return[
    createAnimeProviderManifest({...common,provider_id:'ragf.anime-builtin-generator',display_name:'RAGF Built-in Anime Asset Generator',role:'asset',inputs:['character-genome','episode-intent','seed'],outputs:['anime-character-family','anime-background-family','svg-model-sheets','png-character-media','asset-quality-report'],media:{produces_media:true,media_types:['image/svg+xml','image/png']},character_continuity:{level:'identity-root-and-appearance-root',identity_root_required:true,cross_cut:true,cross_episode:true},motion:{capabilities:['pose-library','expression-library','layered-2d-rig','state-driven-raster']},resources:{cpu:'required',gpu:'not-required',memory_mb:256,disk_mb:64},limits:{timeout_ms:30000,maximum_cost:0},evidence:{outputs:['provider-receipt','asset-lineage','quality-report','media-roots']},quality_boundary:'deterministic experimental Anime assets; commercial source-art quality not proven'}),
