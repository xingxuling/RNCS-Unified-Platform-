...
function logicalEvidencePath(value,label){
  const raw=String(value??'').trim();
  if(!raw)throw Object.assign(new Error(`SVG_RASTER_LOGICAL_PATH_MISSING:${label}`),{code:'SVG_RASTER_LOGICAL_PATH_MISSING',label});
  if(/^[A-Za-z]:[\\/]/.test(raw)||/^\\\\/.test(raw)||/^\/+/.test(raw))throw Object.assign(new Error(`SVG_RASTER_LOGICAL_PATH_ABSOLUTE:${label}`),{code:'SVG_RASTER_LOGICAL_PATH_ABSOLUTE',label,path:raw});
  const normalized=raw.replaceAll('\\','/');
  if(normalized.split('/').some(part=>part==='.'||part==='..'||part===''))throw Object.assign(new Error(`SVG_RASTER_LOGICAL_PATH_TRAVERSAL:${label}`),{code:'SVG_RASTER_LOGICAL_PATH_TRAVERSAL',label,path:raw});
  return normalized;
}
function fileRecord(file,logicalPath){const stat=fs.statSync(file),sha256=createHash('sha256').update(fs.readFileSync(file)).digest('hex');return{path:logicalEvidencePath(logicalPath,'output'),sha256,bytes:stat.size};}
...
export function normalizeSvgRasterBackend(value=process.env.PHASE66_RASTER_BACKEND??(process.platform==='win32'?'resvg-js':'librsvg')){
...
export function rasterizeSvgFile(svgFile,pngFile,{backend=null,provider=null,width,height,rsvgPath=process.env.RSVG_CONVERT_PATH??'rsvg-convert',logicalSvgPath='input.svg',logicalPngPath='output.png'}={}){
  ...
  if(!fs.existsSync(svgFile))throw Object.assign(new Error(`SVG_RASTER_INPUT_MISSING:${svgFile}`),{code:'SVG_RASTER_INPUT_MISSING'});
  const logicalSvg=logicalEvidencePath(logicalSvgPath,'svg'),logicalPng=logicalEvidencePath(logicalPngPath,'png');
...
export function validateSvgRasterReceipt(receipt,{allowedBackends=['librsvg','resvg-js']}={}){
  ...
  for(const [key,label] of [['logical_svg_path','svg'],['logical_png_path','png']]){try{logicalEvidencePath(receipt?.[key],label);}catch(error){errors.push(error.code);}}
  if(receipt?.output?.path!==receipt?.logical_png_path)errors.push('SVG_RASTER_RECEIPT_OUTPUT_PATH_MISMATCH');
  ...
  if(receipt?.receipt_root&&receipt.receipt_root!==rootHash({...receipt,receipt_root:''}))errors.push('SVG_RASTER_RECEIPT_ROOT_MISMATCH');
