...
export function projectFrameGeometry(geometry,camera={}){
  const normalized={width:Number(camera.width??1280),height:Number(camera.height??720),yaw:Number(camera.yaw??0),pitch:Number(camera.pitch??0),scale:Number(camera.scale??1),projection:'orthographic-perspective-hybrid'};
