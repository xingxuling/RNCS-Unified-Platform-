...
function triangleGroup(mesh,triangle){
  ...
  for(const vertexIndex of triangle??[]){
    for(const [bone,weight] of Object.entries(mesh?.bone_weights?.[vertexIndex]??{})){
      for(const [group,bones] of Object.entries(GROUPS))if(bones.has(bone))totals.set(group,(totals.get(group)??0)+Number(weight));
    }
  }
  let best=null,bestWeight=0;for(const [group,weight] of totals)if(weight>bestWeight){best=group;bestWeight=weight;}
  return bestWeight>.04?best:null;
}
...
function boundaryEdges(mask,width,height){
  ...
  for(let y=0;y<height;y+=1)for(let x=0;x<width;x+=1){if(!inside(x,y))continue;
...
function stitchLoops(edges){
  const outgoing=new Map();for(const edge of edges){const k=key(edge[0]),list=outgoing.get(k)??[];list.push(edge);outgoing.set(k,list);}
  ...
  for(const edge of edges){if(used.has(edgeId(edge)))continue;const start=edge[0],loop=[start],current=edge;let guard=0;
    while(current&&guard++<edges.length+8){const id=edgeId(current);if(used.has(id))break;used.add(id);loop.push(current[1]);if(key(current[1])===key(start))break;const candidates=(outgoing.get(key(current[1]))??[]).filter(item=>!used.has(edgeId(item)));current=candidates[0]??null;}
