import {distance2,rootHash,transformPoint} from './canonical.mjs';
...
function nativeGate(certificate,name){return certificate?.gates?.[name]??null;}
function nativeMetric(name,measurement,allowed,method,root,pass){return metric(name,measurement,allowed,method,root,pass);}
function nativeFeaturePoints(asset,posedSkeleton,viewFrame){
  const skull=posedSkeleton?.bones?.find(bone=>bone.id==='skull'),camera=viewFrame?.visibility?.camera??{},attachments=Object.fromEntries((asset?.surface_attachments?.face??[]).map(item=>[item.attachment_id.replace('face:',''),item]));
  return Object.fromEntries(Object.entries(attachments).map(([id,item])=>[id,projectPoint3(transformPoint(skull?.world_transform??{position:[0,0,0],rotation:{}},item.local_position),camera)]));
}

export function measureNativeGeometricTruth({asset,frames=[],certificate=asset?.certificate,evidence_root=null}={}){
  const frameList=Array.isArray(frames)?frames:[],front=frameList.find(frame=>frame.view==='front')??frameList[0]??null,yaw=frameList.find(frame=>frame.view==='three-quarter-right')??front,frontCamera=front?.visibility?.camera??{},assetRoot=asset?.morphology_root??null,frameRoots=frameList.map(frame=>frame.native_surface_root??null),root=evidence_root??rootHash({asset_root:assetRoot,certificate_root:certificate?.certificate_root??null,frame_roots:frameRoots}),gate=name=>nativeGate(certificate,name),ratioGate=gate('projected_anatomical_ratio_validity'),ratioMeasurement=ratioGate?.measurement??null,ratioValues=ratioMeasurement?{head_to_shoulder:ratioMeasurement.head_to_shoulder??ratioMeasurement.head_to_shoulder_ratio??null,head_to_torso:ratioMeasurement.head_to_torso??ratioMeasurement.head_to_torso_ratio??null}:null,frontBones=front?.posed_skeleton?.bones??[],leftShoulder=frontBones.find(bone=>bone.id==='shoulder-left'),rightShoulder=frontBones.find(bone=>bone.id==='shoulder-right'),shoulderDelta=leftShoulder&&rightShoulder?Math.abs(projectPoint3(leftShoulder.world_start,frontCamera)[1]-projectPoint3(rightShoulder.world_start,frontCamera)[1]):null,shoulderDistance=leftShoulder&&rightShoulder?distance2(projectPoint3(leftShoulder.world_start,frontCamera),projectPoint3(rightShoulder.world_start,frontCamera)):null,faceAttachments=Object.fromEntries((asset?.surface_attachments?.face??[]).map(item=>[item.attachment_id.replace('face:',''),item])),noseBridge=faceAttachments.nose_bridge?.local_position,noseTip=faceAttachments.nose_tip?.local_position,mouthCenter=faceAttachments.mouth_center?.local_position,noseMeasurement=noseBridge&&noseTip&&mouthCenter?{nose_bridge_z:noseBridge[2],nose_tip_z:noseTip[2],mouth_z:mouthCenter[2],tip_minus_bridge:noseTip[2]-noseBridge[2],bridge_minus_mouth:noseBridge[2]-mouthCenter[2]}:{available:false},mouthSamples=(front?.visibility?.report?.feature_samples??[]).filter(item=>item.attachment_id.includes('mouth')),mouthYs=mouthSamples.map(item=>item.screen_position?.[1]).filter(finite),mouthDeviation=mouthYs.length?Math.max(...mouthYs)-Math.min(...mouthYs):null,hairRoots=asset?.surface_attachments?.scalp??[],hairGuides=asset?.surface_attachments?.hair_guides??[],silhouettes=frameList.map(frame=>({view:frame.view,islands:frame.visibility?.report?.silhouette_disconnected_islands??null})),featureOcclusion=frameList.map(frame=>({view:frame.view,pass:frame.visibility?.report?.feature_occlusion_correctness??false})),mesh=asset?.canonical_surface_mesh,topology=mesh?.topology_report??{},field=asset?.continuous_morphology_field;
  const nosePass=noseMeasurement.available!==false&&noseMeasurement.tip_minus_bridge>=.002&&noseMeasurement.bridge_minus_mouth>=-.02,shoulderPass=shoulderDelta!==null&&shoulderDistance!==null&&shoulderDelta/Math.max(shoulderDistance,1e-9)<=.02,allFeatureOcclusion=featureOcclusion.length>0&&featureOcclusion.every(item=>item.pass),allSilhouetteConnected=silhouettes.length>0&&silhouettes.every(item=>item.islands===0),components=mesh?.connected_components?.length??null,limbComponents=topology.limb_surface_connected_components??components;
  const metrics=[
    nativeMetric('projected_head_to_shoulder_ratio',{value:ratioValues?.head_to_shoulder??null,source:'MorphologyCertificateV2'}, {min:.35,max:.82},'projected canonical mesh region bounds divided by projected shoulder landmarks',root,gate('projected_anatomical_ratio_validity')?.pass===true&&Number.isFinite(ratioValues?.head_to_shoulder)),
    nativeMetric('head_to_torso_ratio',{value:ratioValues?.head_to_torso??null,source:'MorphologyCertificateV2'}, {min:.45,max:.74},'projected canonical mesh skull bounds divided by torso region bounds',root,gate('projected_anatomical_ratio_validity')?.pass===true&&Number.isFinite(ratioValues?.head_to_torso)),
    nativeMetric('neck_to_skull_connectivity',gate('neck_connection_valid')?.measurement??{available:false},{required:'one canonical field and mesh component'},'NeckField to SkullField continuity and canonical mesh adjacency',root,gate('neck_connection_valid')?.pass===true),
    nativeMetric('neck_to_ribcage_connectivity',{field_components:field?.validation?.connected_components?.length??asset?.field_validation?.connected_components?.length??null,gate:gate('neck_connection_valid')?.measurement??null},{required:'NeckField to SpineField to RibcageField bridge'},'NeckField, SpineField and RibcageField continuous morphology field bridge',root,gate('neck_connection_valid')?.pass===true),
    nativeMetric('shoulder_symmetry_at_neutral',{vertical_delta_px:shoulderDelta,separation_px:shoulderDistance,value:shoulderDelta===null||shoulderDistance===null?null:shoulderDelta/Math.max(shoulderDistance,1e-9)},{max:.02},'neutral posed skeleton shoulder landmarks projected into the front camera',root,shoulderPass),
    nativeMetric('face_feature_surface_containment',{attachment_count:faceAttachments?Object.keys(faceAttachments).length:0,max_normalized_ellipsoid_value:Math.max(...(asset?.surface_attachments?.face??[]).map(item=>item.containment_measurement?.normalized_ellipsoid_value??Infinity))},{max_normalized_ellipsoid_value:1.001},'SurfaceAttachment feature bases evaluated on skull surface before pose and visibility',root,gate('face_feature_containment_valid')?.pass===true),
    nativeMetric('eye_depth_order_under_yaw',yaw?.visibility?.report?.eye_depth_order_under_yaw??{available:false},{depth_delta_abs_min:.00001},'real DepthBuffer feature sample and projected eye depth under three-quarter yaw',root,gate('depth_order_valid')?.pass===true&&Boolean(yaw?.visibility?.report?.eye_depth_order_under_yaw)),
    nativeMetric('nose_depth_relative_to_face',noseMeasurement,{tip_minus_bridge_min:.002,bridge_minus_mouth_min:-.02},'SurfaceAttachment local depth order for nose bridge, tip and mouth plane',root,nosePass),
    nativeMetric('mouth_plane_alignment',{screen_y_min:mouthYs.length?Math.min(...mouthYs):null,screen_y_max:mouthYs.length?Math.max(...mouthYs):null,deviation_px:mouthDeviation},{max_deviation_px:4},'mouth SurfaceAttachment samples projected into one feature plane',root,mouthYs.length>0&&mouthDeviation<=4),
    nativeMetric('hair_root_to_scalp_distance',{root_count:hairRoots.length,max_root_distance:Math.max(...hairRoots.map(item=>item.root_distance_measurement?.root_to_surface_distance??Infinity))},{max_distance:.008},'scalp surface root attachment distance and epsilon',root,gate('hair_root_distance_valid')?.pass===true),
    nativeMetric('hair_mass_connected_to_root',{mass_count:hairGuides.length,curves:hairGuides.reduce((sum,mass)=>sum+(mass.guide_curves?.length??0),0),all_curves_connected:hairGuides.every(mass=>mass.guide_curves?.every(curve=>curve.connected===true))},{required:'every HairRoot reaches guide curve and mass envelope'},'HairRoot to guide curve to mass envelope topology',root,gate('hair_root_distance_valid')?.pass===true&&hairGuides.every(mass=>mass.guide_curves?.every(curve=>curve.connected===true))),
    nativeMetric('hand_to_wrist_connectivity',{hand_weight_vertices:(mesh?.bone_weights??[]).filter(weight=>weight['hand-left']||weight['hand-right']).length,mesh_components:components},{required:'hand and wrist in same canonical surface component'},'hand bone weights and canonical surface component connectivity',root,gate('hand_attachment_valid')?.pass===true),
    nativeMetric('limb_surface_connected_components',{components:limbComponents,mesh_components:components},{max_components:1},'bilateral limb regions in canonical surface adjacency',root,gate('limb_connection_valid')?.pass===true&&components===1),
    nativeMetric('whole_body_connected_components',{components},{max_components:1},'full canonical surface triangle adjacency components',root,components===1),
    nativeMetric('surface_self_intersection_count',{count:topology.surface_self_intersection_count??null},{max:0},'canonical surface non-adjacent triangle intersection measurement',root,Number(topology.surface_self_intersection_count)===0),
    nativeMetric('silhouette_disconnected_islands',{views:silhouettes,max:Math.max(...silhouettes.map(item=>Number(item.islands??Infinity)))},{max:0},'visible silhouette component count after native depth rasterization',root,allSilhouetteConnected),
    nativeMetric('visible_feature_occlusion_correctness',{views:featureOcclusion,tested_buffers:['VisibilityBuffer','DepthBuffer','SurfaceIdBuffer','RegionIdBuffer']},{required:'all tested feature samples visible or correctly occluded'},'feature visibility resolved against native depth and surface ID buffers',root,allFeatureOcclusion),
    nativeMetric('projected_anatomical_ratio_validity',{...ratioValues,shoulder_symmetry:shoulderPass?shoulderDelta/Math.max(shoulderDistance,1e-9):null},{head_to_shoulder:[.35,.82],head_to_torso:[.45,.74],shoulder_symmetry:[0,.02]},'aggregate measured projected anatomy ratios without stylization factors',root,gate('projected_anatomical_ratio_validity')?.pass===true&&shoulderPass)
  ];
  const red_metrics=metrics.filter(item=>!item.pass).map(item=>item.metric);return{format:'rncs.anime-forge-geometric-truth-measurements.v0.2',source:{kind:'native-field-mesh-visibility-static-pack',asset_root:assetRoot,certificate_root:certificate?.certificate_root??null,frame_roots:frameRoots},metrics,red_metrics,status:red_metrics.length?'red':'green',evidence_root:root,measurement_policy:'all metrics require measured field, mesh, attachment, depth, or projection evidence; human visual acceptance remains separate'};
}
