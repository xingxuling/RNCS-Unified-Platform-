...
function buildLowerBodyCertificate(asset){
  ...
  gates.lower_body_foot_attachment=gate('lower_body_foot_attachment',{foot_weights_present:footWeights,lower_regions_present:lowerRegions},{foot_weights:true,regions:true},'surface mesh bone weights and semantic lower-body region coverage',asset.morphology_root,footWeights&&lowerRegions);
  ...
  const failures=Object.entries(gates).filter(([,item])=>item.pass!==true).map(([name])=>name);return seal({format:CERT_FORMAT,version:'0.1.0-alpha.1',asset_root:asset.morphology_root,gates,failures,required_gates:Object.keys(gates),rejection_policy:'any-measured-lower-body-gate-false-rejects-candidate',human_visual_acceptance:'not-automated',certificate_root:''},'certificate_root');
}
...
export function extendArtDirectedMorphologyWithLowerBody(baseAsset,genome,{surface_resolution=null,certificate_mode='canonical'}={}){
  ...
  const meshProfile=meshProfileFor(surface_resolution??baseAsset.mesh_profile??'validation'),mesh=buildCanonicalSurfaceMesh({field,profile:meshProfile}),meshValidation=validateCanonicalSurfaceMesh(mesh);if(!meshValidation.valid)throw Object.assign(new Error(`LOWER_BODY_MESH_REJECTED:${meshValidation.errors.join(',')}`),{code:'LOWER_BODY_MESH_REJECTED',meshValidation});
