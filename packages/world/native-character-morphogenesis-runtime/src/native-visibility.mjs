...
function neighborhoodDepth(depth,x,y,width,height,radius=2,targetDepth=null){
	let best=-Infinity,bestDistance=Infinity;for(let dy=-radius;dy<=radius;dy+=1)for(let dx=-radius;dx<=radius;dx+=1){const nx=x+dx,ny=y+dy;if(nx<0||ny<0||nx>=width||ny>=height)continue;const candidate=Number(depth[indexAt(nx,ny,width)]);if(!finiteDepth(candidate))continue;const distance=targetDepth===null?0:Math.abs(candidate-Number(targetDepth));if(distance<bestDistance||(distance===bestDistance&&candidate>best)){best=candidate;bestDistance=distance;}}
	return best;
}
