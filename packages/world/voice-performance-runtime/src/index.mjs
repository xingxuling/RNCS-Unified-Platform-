...
function waveformMetrics(samples){let peak=0,sumSquares=0;for(const value of samples){const absolute=Math.abs(value);if(absolute>peak)peak=absolute;sumSquares+=value*value}return{peak,rms:Math.sqrt(sumSquares/Math.max(1,samples.length)),sample_count:samples.length}}
