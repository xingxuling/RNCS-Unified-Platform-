...
- Added `createVSRBrowserAssetCache()` as the CacheStorage lowering for the existing VSR asset-loader seam: content-addressed payloads, byte-length/SHA-256 verification, rooted manifest, cold rehydrate, scene/payload revision invalidation, deterministic byte-budget LRU, and inspection diagnostics. Real Chromium evidence covers first write, cold reload hits, and revision rebuild; Reality Build's Android WebView host now reuses the same provider through a stable synthetic origin and has an independent API 35 emulator rehydrate/revision candidate, while quota/performance, remote coherence, and target-device execution remain open.
- `VSRSpatialAssetStreamer` now accepts an optional provider-backed `loadPriority` hint, allowing already-cached asset ids to be rehydrated before a full low-budget catalog scan; dependency/resolution order, asset roots, leases, residency receipts and authority ownership are unchanged. Browser cache inspection also exposes the configured `maxBytes`; VSR tests cover the ordering seam, while Android budget/performance and physical-device evidence remain candidate-only.
- The browser asset provider now exposes `persistAccesses` and `manifestWrites`; concurrent payload writes coalesce manifest persistence while still awaiting the durable flush, and `persistAccesses=false` keeps LRU access telemetry ephemeral without weakening payload byte/SHA/root verification. This is a provider performance/lifecycle lowering only; VSR catalog, resolution, lease, residency and authority semantics are unchanged.
