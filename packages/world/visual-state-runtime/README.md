...
- `createVsrGltfPbrComponentImportHandler()` 将一个或多个 mesh GLB 与按 `material_index`/`mesh_asset_id` 分组的独立四通道 PBR 资源包绑定为可复用的候选组件导入处理器；它要求每个已声明材质的通道完整、通过注入的解码器真实解码，并可消费匹配的 skeleton-rig/animation-clips JSON，验证其确实绑定到 GLB 的 skin 与 animation channels，最后显式返回 consumed/deferred 资源覆盖。glTF 内的多 mesh/多 material 会保留为一个 VSR 场景，多个独立 GLB 会先分别导入再通过隔离命名空间组合，不被压扁成 material 0。
- `createVsrPbrMaterialComponentImportHandler()` 将独立的 RAGF `reality-asset.pbr-texture-pack.v0.4` 四通道 pack（或无 metadata 的四通道资源组）编译为 `vsr.pbr-material-candidate.v0.1`，校验 `pbr/material.json` 的 pack root、通道 MIME/文件根、颜色空间和逻辑到物理资源绑定，并显式返回 material 资源覆盖。它是 candidate-only 的材质表示导入，不解码成目标 GPU 纹理、不写入 RNCS canonical material property，也不替代 AAA 美术验收。
- `createVsrRigAnimationComponentImportHandler()` 将独立的 RAGF skeleton-rig/animation-clips JSON 转成可编译、可验根的 bone-only VSR candidate scene；rig 会生成带父子层级的 joints/skin，animation 会生成 translation、scale 和四元数 rotation channels，缺失 rig 时显式记录 `rig_bound=0`。启用 `fuseGeometry:true` 后，它会按显式 `geometryAssetId` 或 rig/animation 同阶段绑定唯一 GLB，校验 skin joints/父子层级并导入真实蒙皮几何；再启用 `fuseExternalPbr:true` 可复用 glTF/PBR donor 消费同阶段完整四通道外部材质包，`requireExternalPbr:true` 会让缺包或缺通道闭锁；这仍是 candidate import，不把 animation JSON 或本地材质解码冒充成 AAA 质量证明。
- `createVsrParticleComponentImportHandler()` 将 RAGF `rsr.particle-preset.v0.4` JSON 校验并规范化为带 `effectRoot`/`emitterRoot`/`resourceRoot` 的 VSR 粒子发射器 candidate，保留 burst/rate/lifetime/speed/angle/gravity/drag、size/opacity/color 曲线、语义事件和粒子预算，并显式消费粒子 preset、延后同组件的非粒子资源；`lowerVsrParticleEmitterToSpatialScene()` 再把已验证的 emitter 确定性下放为受预算限制的 cross-billboard VSR 场景，交给现有 CPU/WebGPU 空间执行路径。该 lowerer 仍不是 GPU 原生粒子模拟、sprite/flipbook、体积渲染、目标设备质量或 AAA 通过。
- `createVsrNonMeshRepresentationComponentImportHandler()` 为 `sdf`、`voxel`、`point-cloud`、`gaussian-splat`、`neural-field`、`curve` 和通用 `material` descriptor 提供统一的 descriptor + 有序 opaque payload-page 候选导入边界；专用 `createVsrPbrMaterialComponentImportHandler()` 负责可识别的四通道 PBR pack，其余 material descriptor 仍保持通用路径。两条路径都校验身份、bounds/格式或 pack 元数据、总字节数、内容/资源/candidate roots 和 consumed/deferred 覆盖，并接受常见复数 kind 别名；`createVsrNonMeshRepresentationComponentImportHandlerSet()` 可一次性生成非网格 descriptor kind 的显式 handler/verify 绑定，交给 URRF per-kind registry。通用 descriptor 的 `renderStatus` 固定为 `NOT_IMPLEMENTED`。其中采用 `application/vnd.vsr.point-cloud.f32rgba.v0.1` 固定记录格式的 point-cloud candidate，可再通过 `lowerVsrPointCloudCandidateToSpatialScene()` 下放为受预算限制的透明 cross-billboard VSR 场景；采用 `application/vnd.vsr.curve.polyline.f32rgba.v0.1` 固定有序记录格式的 curve candidate，可通过 `lowerVsrCurveCandidateToSpatialScene()` 下放为受预算限制的透明 ribbon 场景；采用带 16 字节网格头和 RGBA8 体素记录的 `application/vnd.vsr.voxel.u8rgba.v0.1` candidate，可通过 `lowerVsrVoxelCandidateToSpatialScene()` 下放为受预算限制的透明/不透明 cube 节点；采用带 16 字节网格头和 little-endian f32 signed-distance 采样的 `application/vnd.vsr.sdf.f32grid.v0.1` candidate，可通过 `lowerVsrSdfCandidateToSpatialScene()` 以受三角形预算限制的 CPU marching-tetrahedra 下放为带梯度法线的表面 mesh；采用 16 字节计数头和 56 字节 f32 center/scale/quaternion/RGBA 记录的 `application/vnd.vsr.gaussian-splat.f32rgba.v0.1` candidate，可通过 `lowerVsrGaussianSplatCandidateToSpatialScene()` 解码并下放为带姿态和尺度的透明 cross-billboard 近似；采用固定 3→16→4 tanh MLP 参数的 `application/vnd.vsr.neural-field.mlp3x16x4.f32.v0.1` candidate，可通过 `lowerVsrNeuralFieldCandidateToSpatialScene()` 在 CPU 逐点求值、采样成有界场并复用表面下放为候选 mesh。neural-field 仍不是任意外部 NeRF、训练/权重质量或 AAA 质量证明；各向异性 Gaussian shader、球谐颜色和目标硬件质量也仍未证明。
- Nearest与Bilinear采样。
- 切线空间Normal Map。
- Alpha Mask在深度写入前裁剪。
