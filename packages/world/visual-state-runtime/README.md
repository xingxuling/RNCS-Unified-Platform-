...
- `createVsrNonMeshRepresentationComponentImportHandler()` 为 `sdf`、`voxel`、`point-cloud`、`gaussian-splat`、`neural-field`、`curve` 和 `material` 提供统一的 descriptor + 有序 opaque payload-page 候选导入边界：校验身份、bounds、格式、总字节数、manifest/content/resource/candidate roots 和 consumed/deferred 覆盖，并接受常见复数 kind 别名；它的 `renderStatus` 固定为 `NOT_IMPLEMENTED`，因此这一步是候选包编译，不是 Gaussian/SDF/体素解码、空间渲染或 AAA 质量证明。
- Nearest与Bilinear采样。
- 切线空间Normal Map。
- Alpha Mask在深度写入前裁剪。
