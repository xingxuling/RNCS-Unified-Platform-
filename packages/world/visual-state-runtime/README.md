...
- `createVsrNonMeshRepresentationComponentImportHandler()` 为 `sdf`、`voxel`、`point-cloud`、`gaussian-splat`、`neural-field`、`curve` 和 `material` 提供统一的 descriptor + 有序 opaque payload-page 候选导入边界：校验身份、bounds、格式、总字节数、manifest/content/resource/candidate roots 和 consumed/deferred 覆盖，并接受常见复数 kind 别名；通用 descriptor 的 `renderStatus` 固定为 `NOT_IMPLEMENTED`。其中采用 `application/vnd.vsr.point-cloud.f32rgba.v0.1` 固定记录格式的 point-cloud candidate，可再通过 `lowerVsrPointCloudCandidateToSpatialScene()` 下放为受预算限制的透明 cross-billboard VSR 场景；Gaussian/SDF/体素/neural-field 仍不是已实现的解码、空间渲染或 AAA 质量证明。
- Nearest与Bilinear采样。
- 切线空间Normal Map。
- Alpha Mask在深度写入前裁剪。
