...
- `createVsrParticleComponentImportHandler()` 将 RAGF `rsr.particle-preset.v0.4` JSON 校验并规范化为带 `effectRoot`/`emitterRoot`/`resourceRoot` 的 VSR 粒子发射器 candidate，保留 burst/rate/lifetime/speed/angle/gravity/drag、size/opacity/color 曲线、语义事件和粒子预算，并显式消费粒子 preset、延后同组件的非粒子资源；它不声称空间粒子渲染、目标设备质量或 AAA 通过。
