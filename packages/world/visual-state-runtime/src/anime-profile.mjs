...
export function createAnimeRenderingProfile({profileId='anime-reference',fps=24,resolution={width:1920,height:1080},style='japanese_tv_anime',quality='anime-tv-reference-v0.1',lineStability='fixed-stroke-raster',colourAnchor='programme-neutral-v1',exposureAnchor='programme-reference-v1'}={}){
  const layerOrder=['background','atmosphere','character','foreground','effects','lighting','correction'],profile={format:VSR_ANIME_RENDERING_PROFILE_FORMAT,version:VSR_ANIME_RENDERING_PROFILE_VERSION,profile_id:profileId,style,quality_boundary:quality,coordinate_space:'anime-canvas-960x540',projection:{kind:'native-2d-and-2.5d',resolution,parallax_layers:true,depth_order:layerOrder.join('-')},camera:{lens_units:'mm',angle_units:'degrees',dolly_units:'normalized',pan_units:'normalized',easing:'stepped-or-explicit',programme_authority:true},line:{stability:lineStability,stroke_model:'fixed-width',shadow_bands:3,anti_aliasing:'deterministic'},exposure:{fps,implicit_interpolation:false,default_policy:'on_twos',anchor:exposureAnchor},compositing:{premultiplied_alpha:false,layer_order:layerOrder,color_space:'srgb',colour_anchor:colourAnchor,crop_policy:'programme-canvas',mask_policy:'bounded-layer-masks',cut_seam_policy:'validate-colour-exposure-character-camera'},authority:{source:'RNCS',scope:'episode-composition',external_provider_required:false,mode:'reference'}};
...
export function validateAnimeRenderingProfile(profile){
  const errors=[],expectedLayers=['background','atmosphere','character','foreground','effects','lighting','correction'];if(profile?.format!==VSR_ANIME_RENDERING_PROFILE_FORMAT)errors.push('VSR_ANIME_PROFILE_FORMAT_INVALID');if(profile?.version!==VSR_ANIME_RENDERING_PROFILE_VERSION)errors.push('VSR_ANIME_PROFILE_VERSION_INVALID');if(!profile?.profile_root)errors.push('VSR_ANIME_PROFILE_ROOT_MISSING');if(profile?.line?.stability!=='fixed-stroke-raster')errors.push('VSR_ANIME_LINE_STABILITY_INVALID');if(profile?.exposure?.implicit_interpolation!==false)errors.push('VSR_ANIME_IMPLICIT_INTERPOLATION_ENABLED');if(JSON.stringify(profile?.compositing?.layer_order)!==JSON.stringify(expectedLayers))errors.push('VSR_ANIME_LAYER_ORDER_INVALID');if(profile?.compositing?.crop_policy!=='programme-canvas'||profile?.compositing?.mask_policy!=='bounded-layer-masks')errors.push('VSR_ANIME_COMPOSITION_POLICY_INVALID');if(profile?.authority?.scope!=='episode-composition')errors.push('VSR_ANIME_EPISODE_AUTHORITY_REQUIRED');return{valid:errors.length===0,errors,profile_root:profile?.profile_root??null};
}
