...
This alpha proves a 9×9 region by default, a deterministic two-instance replication loop, a JSON restart boundary that restores the replication idempotency ledger, a bounded authenticated packet link with loss/retry behavior, a deterministic same-base multi-writer conflict court, and an atomic durable-bundle store with crash-point recovery. It does not claim an MMO-scale distributed world, production network security, consensus, GPU generation, external Spark execution, or production delivery.
