...
`createSpatialScene({selection})` lowers that envelope and the current active chunks into a rooted `vsr.spatial-scene.v0.4` scene with the `rncs.large-world-spatial-scene.v0.1` extension. The lowering includes decimated proxy terrain, standard terrain, structures, resources, streaming cells, lights, and browser-consumable asset references. `verifyLargeWorldSpatialScene` checks the lowering root and authority boundary. The result is a VSR projection input; it does not promote representations or write canonical world state.
