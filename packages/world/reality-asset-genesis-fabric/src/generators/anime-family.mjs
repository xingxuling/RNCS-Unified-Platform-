...
function normalizeMotion(input={}){
  const value=input??{},fps=boundedInteger(value.fps,12,1,60),frameCount=boundedInteger(value.frame_count??value.frames,8,2,24),defaultBlink=[Math.max(1,Math.floor(frameCount*.375)),Math.max(1,Math.floor(frameCount*.5))],blinkFrames=[...new Set((Array.isArray(value.blink_frames)?value.blink_frames:defaultBlink).map(frame=>boundedInteger(frame,0,0,frameCount-1)))].sort((a,b)=>a-b);
  return{fps,frame_count:frameCount,duration_seconds:Math.round(frameCount/fps*1000)/1000,blink_frames:blinkFrames,channels:['hair','coat','breathing','blink'],deterministic:true,source:'ragf.anime-builtin-generator'};
}

function sampleMotion(config,frame){
  const phase=config.frame_count<=1?0:(frame/(config.frame_count-1))*Math.PI*2,blink=config.blink_frames.includes(frame);
...
export function validateAnimeCharacterFamily(family){
  ...
  const sequence=family?.media_manifest?.motion_sequence;if(!sequence||sequence.frame_count!==sequence.frames?.length||sequence.frame_count<2||sequence.motion_root!==family?.motion_root)errors.push('RAGF_ANIME_CHARACTER_MOTION_SEQUENCE_INVALID');
