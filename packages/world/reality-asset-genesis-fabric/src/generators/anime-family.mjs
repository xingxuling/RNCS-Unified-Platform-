...
const motionNumber=value=>{const number=Number(value);return Number.isFinite(number)?number:0};

function rasterCharacter({slots,proportions,view='front',state}){
  const background=parseHex('#eef2f5'),ink=parseHex('#17253d'),skin=parseHex(slots.skin),hair=parseHex(slots.hair),base=parseHex(slots.base),secondary=parseHex(slots.secondary),trim=parseHex(slots.trim),eye=parseHex(slots.eye),emblem=parseHex(slots.emblem),s=surface(240,320,background),face=faceGeometry(proportions,view),expression=expressionGeometry(state,proportions),back=view==='back',side=view==='side',secondaryMotion=state.secondary_motion??{},breath=motionNumber(secondaryMotion.breathing),hairSway=motionNumber(secondaryMotion.hair),coatSway=motionNumber(secondaryMotion.coat);
  const shoulder=62+proportions.shoulder_width*48,torso=52+proportions.torso_length*34,leg=38+proportions.limb_ratio*20,raise=/raise|gesture|begin/i.test(state.pose),combat=/combat|attack/i.test(state.pose),armY=raise?175:combat?205:224;
  const armSpread=(raise?48:combat?62:42)+coatSway,bodyY=breath,headY=breath;
  ellipse(s,120,299,78,8,parseHex('#cbd2d8'));line(s,92,224+bodyY,90,260+leg+bodyY,base,22);line(s,148,224+bodyY,150,260+leg+bodyY,base,22);line(s,78,260+leg+bodyY,103,260+leg+bodyY,ink,10);line(s,137,260+leg+bodyY,162,260+leg+bodyY,ink,10);line(s,120-shoulder+5,151+bodyY,120-armSpread,armY+bodyY,secondary,18);line(s,120+shoulder-5,151+bodyY,120+armSpread,armY+bodyY,secondary,18);ellipse(s,120-armSpread,armY+8+bodyY,10,10,skin);ellipse(s,120+armSpread,armY+8+bodyY,10,10,skin);polygon(s,[[120-shoulder,137+bodyY],[120,126+bodyY],[120+shoulder,137+bodyY],[120+torso,224+bodyY],[120-torso,224+bodyY]],base);polygon(s,[[95,137+bodyY],[120,166+bodyY],[145,137+bodyY],[120,151+bodyY]],trim);line(s,120,151+bodyY,120,217+bodyY,emblem,3);polygon(s,[[120,174+bodyY],[130,184+bodyY],[120,198+bodyY],[110,184+bodyY]],emblem);rect(s,120-torso+8,185+bodyY,(torso-8)*2,28,secondary);ellipse(s,face.cx,face.cy+headY,face.rx,face.ry,skin);
  if(back){polygon(s,[[face.cx-face.rx+hairSway,86+headY],[face.cx-face.rx+4+hairSway,18+headY],[face.cx+hairSway,20+headY],[face.cx+face.rx-4+hairSway,18+headY],[face.cx+face.rx+hairSway,86+headY],[face.cx+26+hairSway,142+headY],[face.cx-27+hairSway,142+headY]],hair);}else{ellipse(s,face.cx,82+headY,side?4:7,expression.eyeRy,eye);if(!side)ellipse(s,face.cx+26,82+headY,7,expression.eyeRy,eye);circle(s,face.cx+1+state.gaze_x*2,82+state.gaze_y*3+headY,2,ink);if(!side)circle(s,face.cx+27+state.gaze_x*2,82+state.gaze_y*3+headY,2,ink);line(s,face.cx-24,70+headY,face.cx-4,expression.question?62+headY:68+headY,ink,4);if(!side)line(s,face.cx+4,expression.question?62+headY:68+headY,face.cx+24,70+headY,ink,4);if(expression.mouthOpen)ellipse(s,face.cx,112+headY,8,expression.mouthRy,ink);else line(s,face.cx-8,110+headY,face.cx+8,110+headY,ink,3);polygon(s,[[face.cx-face.rx+hairSway,80+headY],[face.cx-face.rx+4+hairSway,21+headY],[face.cx+hairSway,20+headY],[face.cx+face.rx-4+hairSway,21+headY],[face.cx+face.rx+hairSway,80+headY],[face.cx+25+hairSway,65+headY],[face.cx+12+hairSway,112+headY],[face.cx+2+hairSway,63+headY],[face.cx-16+hairSway,118+headY],[face.cx-18+hairSway,58+headY],[face.cx-37+hairSway,103+headY]],hair);}
...
const boundedInteger=(value,fallback,min,max)=>{const number=Number(value);return Number.isFinite(number)?Math.max(min,Math.min(max,Math.round(number))):fallback};
const roundMotion=value=>Math.round(motionNumber(value)*1000)/1000;

function normalizeMotion(input={}){
  const value=input??{},fps=boundedInteger(value.fps,12,1,60),frameCount=boundedInteger(value.frame_count??value.frames,8,2,24),loopMode=value.loop_mode==='hold'?'hold':'cycle',loopPeriodFrames=boundedInteger(value.loop_period_frames,loopMode==='cycle'?Math.max(2,frameCount-1):frameCount,2,frameCount),defaultBlink=[Math.max(1,Math.floor(frameCount*.375)),Math.max(1,Math.floor(frameCount*.5))],blinkFrames=[...new Set((Array.isArray(value.blink_frames)?value.blink_frames:defaultBlink).map(frame=>boundedInteger(frame,0,0,frameCount-1)))].sort((a,b)=>a-b);
  return{fps,frame_count:frameCount,duration_seconds:Math.round(frameCount/fps*1000)/1000,loop_mode:loopMode,loop_period_frames:loopPeriodFrames,loop_period_seconds:Math.round(loopPeriodFrames/fps*1000)/1000,blink_frames:blinkFrames,channels:['hair','coat','breathing','blink'],deterministic:true,source:'ragf.anime-builtin-generator'};
}

function sampleMotion(config,frame){
  const phase=config.frame_count<=1?0:(frame/(config.loop_mode==='cycle'?config.loop_period_frames:Math.max(1,config.frame_count-1)))*Math.PI*2,blink=config.blink_frames.includes(frame);
  return{frame_number:frame,time_seconds:Math.round(frame/config.fps*1000)/1000,blink,eye_state:blink?'blink':'open',secondary_motion:{frame,hair:roundMotion(Math.sin(phase+.35)*.9),coat:roundMotion(Math.sin(phase+.8)*1.2),breathing:roundMotion(Math.sin(phase)*1.35),foreground:0}};
}

function generateAnimeCharacterFamilyBase({assetId='character:blue-tianlin',name='蓝天临',palette,seed='anime-reference-v0.1',genome=null,identitySignature=null,state={}}={}){
...
export function generateAnimeCharacterFamily(input={}){
  const base=generateAnimeCharacterFamilyBase(input),config=normalizeMotion(input.motion),motionFrames=[],motionFiles={};
  for(let frame=0;frame<config.frame_count;frame++){
    const sample=sampleMotion(config,frame),frameState={...base.family.state,eye_state:sample.eye_state,secondary_motion:sample.secondary_motion},raster=rasterCharacter({slots:base.family.palette,proportions:base.family.proportions,view:'front',state:frameState}),fileName=`motion/frame-${String(frame+1).padStart(4,'0')}.png`,file=mediaFile(fileName,raster.png.toString('base64'),'image/png','base64',raster.png.length),frameStateRoot=rootHash({base_state_root:base.family.state_root,sample});
    motionFiles[fileName]=file;
    motionFrames.push({...sample,file:fileName,file_root:file.root,byte_length:file.byte_length,width:raster.width,height:raster.height,non_background_ratio:raster.non_background_ratio,state_root:frameStateRoot,motion_state_root:rootHash({frame:sample.frame_number,eye_state:sample.eye_state,secondary_motion:sample.secondary_motion})});
  }
  const motionTrack=seal({format:'ragf.anime-motion-track.v0.1',version:'0.1.0',asset_id:base.family.identity.asset_id,identity_root:base.family.identity_root,palette_root:base.family.palette_root,proportion_root:base.family.proportion_root,appearance_root:base.family.continuity_bundle.appearance_root,fps:config.fps,frame_count:config.frame_count,duration_seconds:config.duration_seconds,loop_mode:config.loop_mode,loop_period_frames:config.loop_period_frames,loop_period_seconds:config.loop_period_seconds,deterministic:true,tracks:[{track_id:'secondary-motion.hair',layer:'hair',property:'hair',mode:'sine-sampled',amplitude:.9,frequency:'one-cycle',phase:.35},{track_id:'secondary-motion.coat',layer:'coat',property:'coat',mode:'sine-sampled',amplitude:1.2,frequency:'one-cycle',phase:.8},{track_id:'secondary-motion.breathing',layer:'body',property:'breathing',mode:'sine-sampled',amplitude:1.35,frequency:'one-cycle',phase:0},{track_id:'facial.blink',layer:'eyes',property:'eye_state',mode:'stepped',frames:config.blink_frames}],frames:motionFrames.map(frame=>({frame_number:frame.frame_number,time_seconds:frame.time_seconds,file:frame.file,motion_state_root:frame.motion_state_root,eye_state:frame.eye_state,secondary_motion:frame.secondary_motion})),continuity:{stable_identity:true,stable_palette:true,stable_proportions:true,stable_appearance:true},authority:'candidate-only',quality_boundary:'deterministic experimental secondary motion with explicit cycle coverage; physical cloth and hair simulation not claimed',motion_root:''},'motion_root');
  const motionTrackFile=mediaFile('motion-track.json',`${JSON.stringify(motionTrack,null,2)}\n`,'application/json'),files={...base.files,...motionFiles,'motion-track.json':motionTrackFile},mediaRoots=Object.fromEntries(Object.entries(files).map(([file,content])=>[file,content.root])),assetRoot=rootHash(mediaRoots),motionRoots=motionFrames.map(frame=>frame.file_root),quality=seal({...base.family.quality,format:'ragf.anime-asset-quality-report.v0.3',rendered_media:{...base.family.quality.rendered_media,motion_png_sequence:true,motion_frame_count:config.frame_count,motion_fps:config.fps,motion_track_file:'motion-track.json',unique_motion_frame_roots:new Set(motionRoots).size,temporal_variation:new Set(motionFrames.map(frame=>frame.state_root)).size>1},semantic_role_count:Object.keys(files).length,temporal:{frame_count:config.frame_count,fps:config.fps,unique_frame_roots:new Set(motionRoots).size,unique_state_roots:new Set(motionFrames.map(frame=>frame.state_root)).size,blink_frames:config.blink_frames,secondary_motion_channels:config.channels,loop_mode:config.loop_mode,loop_period_frames:config.loop_period_frames,loop_period_seconds:config.loop_period_seconds,coverage_policy:config.loop_mode==='cycle'?'cycle-by-timebase':'hold-last'},media_roots:mediaRoots,boundary:'deterministic experimental character asset with explicit cycle coverage; commercial Anime source-art quality and physical simulation not proven',quality_root:''},'quality_root'),providerInputRoot=rootHash({genome_root:base.family.genome_root,identity_root:base.family.identity_root,palette_root:base.family.palette_root,proportion_root:base.family.proportion_root,appearance_root:base.family.continuity_bundle.appearance_root,motion:{fps:config.fps,frame_count:config.frame_count,loop_mode:config.loop_mode,loop_period_frames:config.loop_period_frames,blink_frames:config.blink_frames}}),providerReceipt=seal({...base.family.provider_receipt,provider_version:'0.5.0',capability:'anime-character-family-with-motion-loop',input_root:providerInputRoot,output_root:assetRoot,real_media:true,media_types:['image/svg+xml','image/png','image/png-sequence','application/json'],quality_boundary:quality.boundary,receipt_root:''},'receipt_root'),family=clone(base.family);
  family.version='0.4.0-alpha.1';family.asset_root=assetRoot;family.version_root=rootHash({identity:family.identity,appearance:family.appearance,proportions:family.proportions,face_features:family.face_features,palette:family.palette,motion_root:motionTrack.motion_root,quality_root:quality.quality_root});family.quality=quality;family.motion_root=motionTrack.motion_root;family.motion_track=motionTrack;family.render_contract={...family.render_contract,frame_rate:config.fps,frame_count:config.frame_count,duration_seconds:config.duration_seconds,motion_track:'motion-track.json',motion_sequence_pattern:'motion/frame-####.png',secondary_motion_fields:['hair','coat','breathing'],temporal_deterministic:true};family.media_manifest={...family.media_manifest,motion_root:motionTrack.motion_root,motion_sequence:{format:'ragf.anime-motion-sequence.v0.1',fps:config.fps,frame_count:config.frame_count,duration_seconds:config.duration_seconds,frames:motionFrames.map(frame=>({frame_number:frame.frame_number,time_seconds:frame.time_seconds,file:frame.file,mime:'image/png',byte_length:frame.byte_length,root:frame.file_root,state_root:frame.state_root,motion_state_root:frame.motion_state_root,eye_state:frame.eye_state,secondary_motion:frame.secondary_motion})),motion_root:motionTrack.motion_root},state_root:family.state_root};family.continuity_bundle={...family.continuity_bundle,motion_root:motionTrack.motion_root,stable_across_cuts:true};family.lineage_graph={...family.lineage_graph,provider_root:providerReceipt.receipt_root};family.provider_receipt=providerReceipt;family.provider={...family.provider,version:'0.4.0',capabilities:['state-driven-raster','deterministic-motion-sequence','secondary-motion-track']};
  family.render_contract={...family.render_contract,motion_loop_mode:config.loop_mode,motion_loop_period_frames:config.loop_period_frames};
  family.media_manifest={...family.media_manifest,motion_sequence:{...family.media_manifest.motion_sequence,loop_mode:config.loop_mode,loop_period_frames:config.loop_period_frames,loop_period_seconds:config.loop_period_seconds}};
  family.provider={...family.provider,version:'0.5.0',capabilities:['state-driven-raster','deterministic-motion-sequence','loopable-motion-sequence','secondary-motion-track']};
  return{family:seal(family,'family_root'),files};
}
...
export function validateAnimeCharacterFamily(family){
  ...
  if(family?.version!=='0.4.0-alpha.1'||!family?.motion_root||!verifySeal(family?.motion_track,'motion_root'))errors.push('RAGF_ANIME_CHARACTER_MOTION_TRACK_INVALID');
  const sequence=family?.media_manifest?.motion_sequence,motionTrack=family?.motion_track;if(!sequence||sequence.frame_count!==sequence.frames?.length||sequence.frame_count<2||sequence.motion_root!==family?.motion_root)errors.push('RAGF_ANIME_CHARACTER_MOTION_SEQUENCE_INVALID');
  if(!['cycle','hold'].includes(motionTrack?.loop_mode)||!Number.isInteger(motionTrack?.loop_period_frames)||motionTrack.loop_period_frames<2||motionTrack.loop_period_frames>Number(motionTrack?.frame_count??0))errors.push('RAGF_ANIME_CHARACTER_MOTION_COVERAGE_INVALID');
  if(sequence?.loop_mode!==motionTrack?.loop_mode||sequence?.loop_period_frames!==motionTrack?.loop_period_frames||family?.quality?.temporal?.coverage_policy!==(motionTrack?.loop_mode==='cycle'?'cycle-by-timebase':'hold-last'))errors.push('RAGF_ANIME_CHARACTER_MOTION_COVERAGE_DRIFT');
  if(family?.quality?.temporal?.unique_frame_roots<2||family?.quality?.rendered_media?.motion_png_sequence!==true)errors.push('RAGF_ANIME_CHARACTER_TEMPORAL_VARIATION_MISSING');
