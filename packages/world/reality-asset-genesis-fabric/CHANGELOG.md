...
- Added a profile-aware deterministic `creature` reference candidate with a quadruped mesh, 13-bone rig, four gait/combat clips, tail and footstep events, dynamic RSR/collision contracts, quadruped retarget map, and creature-specific 2D projections. It remains bounded local structural evidence rather than high-resolution or AAA production proof.
- Added a dedicated VFX reference Provider and sealed `ragf.vfx-asset-contract.v0.1`: particle, volume, flipbook, and curve representations share a world-time effect graph, fixed-step budgets, platform/runtime contracts, artifact roots, and fail-closed verification. The route contains no mesh shortcut and remains candidate evidence, not GPU-native, target-device, human-reviewed, or AAA delivery.
- Added a TRELLIS.2 local candidate adapter with sealed command/weights/CUDA/dependency/license/timeout/result-root preflight evidence and fail-closed execution; missing external runtime remains `BLOCKED / NOT_RUN`.
- Added the RNCS-owned URRF v0.3 coverage matrix for all 26 success criteria and nine K400 gates, with repository-resolved source/schema/test/evidence references and explicit sensor/AI/AAA blockers.
