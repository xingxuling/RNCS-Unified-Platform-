...
- 每个真实 PNG 运动帧同时保留 `sha256-rgba8-canonical` 像素根；PNG 文件根与原始 RGBA 像素根分开记录，避免只用容器字节或帧编号推断画面变化。
- motion track 保存相邻帧 RGBA 差异、循环端点差异和眨眼邻帧变化；`motion_visual_quality_root` 可由轨道元数据重新计算。
- 视觉质量门要求活动周期有足够唯一像素根、相邻帧有真实变化、眨眼产生画面变化，并要求循环首尾 RGBA 差为零。
- 该门证明状态确实进入内置确定性光栅媒体，不等同于商业动画美术质量、专业 DCC、物理布料/头发模拟或人工审美验收。
