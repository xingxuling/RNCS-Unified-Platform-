...
export function createAnimeSecondaryMotionProfile({profileId='anime-secondary-reference',seed='rsr-anime-reference-v0.1',hair='subtle',coat='subtle',directorOverrides=[]}={}){
  const profile={format:RSR_ANIME_SECONDARY_MOTION_FORMAT,version:RSR_ANIME_SECONDARY_MOTION_VERSION,profile_id:profileId,seed,layer_bindings:{hair:{mode:hair,amplitude:hair==='none'?0:.9,frequency:.27,phase:.11},coat:{mode:coat,amplitude:coat==='none'?0:1.2,frequency:.19,phase:.7}},director_overrides:[...(directorOverrides??[])],override_policy:{authority:'director',bounded:true,requires_frame_range:true,requires_reason:true,unknown_layer:'reject'},continuity:{stable_asset_identity:true,deterministic_seed:true},quality_boundary:'reference secondary motion; physical cloth/hair simulation not claimed'};
...
export function applyDirectorMotionOverride(profile,override){
  if(!['hair','coat'].includes(override?.layer))throw Object.assign(new Error(`RSR_MOTION_LAYER_UNSUPPORTED:${override?.layer}`),{code:'RSR_MOTION_LAYER_UNSUPPORTED'});
...
export function resolveAnimeSecondaryMotion(profile,{frame=0}={}){
  ...
  const values={hair:clamp(base.hair??0,-12,12),coat:clamp(base.coat??0,-12,12),hair_override:base.hair_override,coat_override:base.coat_override,profile_root:profile?.profile_root??null,frame,authority:'director-overridable-reference'};return{...values,motion_root:rootHash(values)};
}

export function validateAnimeSecondaryMotionProfile(profile){const errors=[];if(profile?.format!==RSR_ANIME_SECONDARY_MOTION_FORMAT)errors.push('RSR_ANIME_MOTION_FORMAT_INVALID');if(profile?.version!==RSR_ANIME_SECONDARY_MOTION_VERSION)errors.push('RSR_ANIME_MOTION_VERSION_INVALID');if(!profile?.profile_root)errors.push('RSR_ANIME_MOTION_ROOT_MISSING');for(const item of profile?.director_overrides??[]){if(!['hair','coat'].includes(item.layer))errors.push(`RSR_ANIME_MOTION_LAYER:${item.layer}`);if(item.end_frame<item.start_frame)errors.push(`RSR_ANIME_MOTION_RANGE:${item.override_id}`);if(item.authority!=='director')errors.push(`RSR_ANIME_MOTION_AUTHORITY:${item.override_id}`)}return{valid:errors.length===0,errors,profile_root:profile?.profile_root??null};}
