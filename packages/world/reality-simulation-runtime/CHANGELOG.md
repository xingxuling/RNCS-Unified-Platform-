...
- 新增 bounded `patch-heightfield` runtime command：按有序整数样本和可选 expected heightfield root 执行静态/固定旋转运动学 terrain patch，输出可校验 mutation root，精确失效受影响 contact cache，并进入 snapshot/replay/causal delta；Spatial Embodiment 达到 76/76、RSR 全量达到 210/210。动态 body terrain、Large World deform stream、RCL/Kernel command lowering、连续完整 manifold 和目标设备证据仍保持 OPEN。
