...
- 新增 RCL native spatial command plan 与独立 `@taowind/rcl-rsr-spatial-bridge`：`rncs.spatial.command.<alias>.*` 经 native authority plan、root-bound lowering receipt 后进入既有 RSR/VSR candidate session，并完成模拟/重放回归（RCL control plane 17/17、bridge 2/2）；一等 RCL spatial primitive、Kernel command batch、动态 terrain、Large World deform scheduler、连续完整 manifold 和目标设备证据仍保持 OPEN。
