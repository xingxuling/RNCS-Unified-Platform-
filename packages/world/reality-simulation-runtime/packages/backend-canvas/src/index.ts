...
export class PixelSurface {
  ...
  fillRect(rect:VSRRect,paint:VSRPaint|undefined,opacity=1,clips?:VSRClip[]):void {const minX=Math.max(0,Math.floor(rect.x)),maxX=Math.min(this.width,Math.ceil(rect.x+rect.width)),minY=Math.max(0,Math.floor(rect.y)),maxY=Math.min(this.height,Math.ceil(rect.y+rect.height));for(let y=minY;y<maxY;y++)for(let x=minX;x<maxX;x++)this.blend(x,y,paintColor(paint,x+.5,y+.5),opacity,clips);}
  ...
  fillTransformedRect(rect:VSRRect,matrix:VSRMatrix3,paint:VSRPaint|undefined,opacity=1,clips?:VSRClip[],radius=0):void {const r=Math.max(0,Math.min(Math.min(rect.width,rect.height)/2,radius));this.fillByInverse(rect,matrix,(x,y)=>{if(x<rect.x||y<rect.y||x>rect.x+rect.width||y>rect.y+rect.height)return false;if(r<=0)return true;const cx=x<rect.x+r?rect.x+r:x>rect.x+rect.width-r?rect.x+rect.width-r:x,cy=y<rect.y+r?rect.y+r:y>rect.y+rect.height-r?rect.y+rect.height-r:y;return(x-cx)**2+(y-cy)**2<=r*r;},paint,opacity,clips);}
...
export function encodePng(surface:PixelSurface):Uint8Array {const raw=Buffer.alloc((surface.width*4+1)*surface.height);for(let y=0;y<surface.height;y++){const row=y*(surface.width*4+1);raw[row]=0;Buffer.from(surface.data.buffer,surface.data.byteOffset+y*surface.width*4,surface.width*4).copy(raw,row+1);}const header=Buffer.alloc(13);header.writeUInt32BE(surface.width,0);header.writeUInt32BE(surface.height,4);header[8]=8;header[9]=6;header[10]=0;header[11]=0;header[12]=0;return Buffer.concat([Buffer.from([137,80,78,71,13,10,26,10]),chunk('IHDR',header),chunk('IDAT',deflateSync(raw,{level:9})),chunk('IEND',Buffer.alloc(0))]);}
export function renderPng(state:VSRDisplayState,resources?:VSRRasterResourceInput):Uint8Array {return encodePng(rasterizeDisplayState(state,resources));}
