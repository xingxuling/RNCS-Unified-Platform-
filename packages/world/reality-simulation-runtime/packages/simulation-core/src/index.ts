...
export const RSR_VERSION = '0.2.0-alpha.1';
...
export class DeterministicSimulationWorld {
  ...
  constructor(config: SimulationWorldConfig, identity?: { configHash?: string }) {
    ...
    this.configHash = identity?.configHash ?? semanticHash(configSemanticView(config));
  ...
  snapshot(): SimulationSnapshot {
    ...
    const contactRoot = semanticHash(contacts.map(contactSemanticView));
    const stateView = {
    ...
    return {
      ...
      solverIterations: this.solverIterations, configHash: this.configHash,
      bodies, contacts, events, contactRoot, stateRoot: semanticHash(stateView)
    };
  }
}
...
export function snapshotToCausalDelta(snapshot: SimulationSnapshot, baseRealityRoot: string): RFECausalDelta {
  ...
  for (const body of snapshot.bodies) {
    const evidence = semanticHash({ worldId: snapshot.worldId, tick: snapshot.tick, body: bodySemanticView(body), stateRoot: snapshot.stateRoot });
