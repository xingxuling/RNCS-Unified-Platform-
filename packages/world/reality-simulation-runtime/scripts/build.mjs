...
try {
  ...
  console.error('RSR_BUILD_TYPESCRIPT_MISSING: run `npm ci` from the RNCS repository root.');
  process.exit(1);
}
...
if (result.error) {
  console.error(`RSR_BUILD_TYPESCRIPT_SPAWN_FAILED: ${result.error.message}`);
  process.exit(1);
}
