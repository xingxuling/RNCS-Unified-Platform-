...
- 新增 RCL native spatial command plan 与 `@taowind/rcl-rsr-spatial-bridge` candidate lowering：RCL `Sequence` 样本声明经 `rncs.rcl-spatial-command-plan.v0.1` 和 lowering root 校验后进入现有 RSR/VSR session，候选模拟保持正式状态不变并可确定性 replay；该路径不等于一等 RCL primitive、Kernel command batch、dynamic terrain、Large World deform scheduler、目标设备或生产性能证明。
