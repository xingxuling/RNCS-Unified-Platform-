...
- 新增 bounded `patch-heightfield` runtime command：对静态或固定旋转运动学 heightfield 以有序整数样本执行受限原子 patch，支持 expected heightfield root、mutation root、因地形变化精确失效接触缓存，并随 snapshot/replay 与 causal delta 输出；RCL native control plan → rooted Kernel candidate command batch → RSR/VSR candidate lowering 已由独立 bridge 接通，但一等 RCL spatial primitive、动态 body heightfield 和完整 terrain transaction 仍保持显式边界。
...
- Convex Hull 当前是单个作者网格 fixture；自动凸分解、车辆轮胎接触和完整工业接触流形仍未实现。`heightfield` 仍是 candidate provider：仅支持未旋转静态/运动学场，在单个 AABB penetration test 之上提供最多四点 bounded support manifold、bounded authored-surface ray query、bounded sphere-vs-authored-triangle sweep、bounded upright-capsule-vs-authored-triangle sweep，以及最多 4096 个样本的 runtime patch；它不是连续完整三角形接触流形，不能接受动态 body heightfield，也没有 Large World deform stream、物理设备性能或生产部署证据。
- 接触缓存是确定性单点持久流形，不是完整四点/八点工业接触流形。
- 约束求解器仍是单线程整数确定性参考实现，不声称达到Jolt/PhysX/Chaos吞吐量。
- Kernel binding 当前是一实体一 body/fixture 的确定性桥；多 fixture archetype、完整 ECS chunk/processor、并行 Job 和网络 wire frame 绑定仍未完成。Heightfield patch 与 velocity correction 已有 RSR-native command path、RCL physical command profile、Kernel candidate command batch 和独立的 RCL → RSR/VSR candidate lowering；一等 RCL spatial primitive、heightfield 与 managed residency 仍是 bounded candidate，不是生产级 terrain streaming、连续完整 swept terrain manifold、动态 terrain transaction 或物理设备性能证明。
