...
- Convex Hull 当前是单个作者网格 fixture；自动凸分解、车辆轮胎接触和完整工业接触流形仍未实现。`heightfield` 仍是 candidate provider：仅支持未旋转静态/运动学场和单个 AABB 支撑碰撞，并新增 bounded authored-surface ray query、bounded sphere-vs-authored-triangle sweep 与 bounded upright-capsule-vs-authored-triangle sweep；它不是完整三角形接触流形，也没有动态地形或物理设备性能证据。
