...
async function makeEnvelope({manifest,intentId,intent,actor,payload,authorityPlan,host}) {
...
export async function bindAndExecuteIntent({manifest,files,graph,intentId,host,subject,payload={},gatewayUrl='./api',confirmHighRisk=async()=>false,emit=()=>{}}) {
  const section=manifest.capability_binding; if(section?.version!=='0.7')throw new Error('胶囊没有声明 capability_binding.version=0.7');
  ...
  const authorityPlan=await authorityNegotiation(request,negotiation,chain), intent=graph.intents[intentId], envelope=await makeEnvelope({manifest,intentId,intent,actor,payload,authorityPlan,host});
  ...
  const attempts=[]; let output=null, selected=null;
  ...
  const receipt=await seal({format:'hnaf.intent-execution-receipt.v0.7',version:'0.7',status:'executed',intent_id:intentId,selected_provider:selected.provider_id,selected_capability:selected.capability_id,attempts,output,request_root:negotiation.request.request_root,plan_root:authorityPlan.plan.plan_root,decision_root:decision.decision_root,envelope_root:envelope.envelope_root,executed_at:new Date().toISOString(),backend:{negotiation:'reality-one-gateway',authority:'reality-one-gateway',execution:'hnaf-web-builtin'}},'receipt_root');
  return {status:'executed',intent_id:intentId,binding:{request_root:negotiation.request.request_root,plan_root:authorityPlan.plan.plan_root,decision_root:decision.decision_root},authority:decision,approval,receipt,state:output};
}
