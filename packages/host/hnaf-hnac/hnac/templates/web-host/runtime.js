...
const RUNTIME_VERSION = '0.8.0-web';
...
export async function runCapsule(arrayBuffer, emit = () => {}, options = {}) {
  ...
  const snapshot = (verified.manifest.state?.autosnapshot ?? ['0.5', '0.6', '0.7', '0.8'].includes(verified.manifest.format_version)) ? await broker.snapshot('capsule-run') : null;
