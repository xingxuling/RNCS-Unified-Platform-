...
- URRF/Reality Transport Fabric semantic seam：Loopback 将 `input/ack/rejection/delta/snapshot` 分别映射为 `CONTROL/ACK/NACK/STATE_DELTA` candidate packets，保留 profile/packet/fabric roots；该绑定不获得 canonical world write authority。
...
这条路径已用序列化 JSON、独立 Node 进程、临时文件同步/原子 rename、主文件优先和注入式故障恢复验证；它仍不是实际断电、WAN failover、TLS、跨节点 leader 选举、密钥托管或生产会话恢复证明。共享存储只提供候选 artifact 持久化能力，不授予 canonical state 写权限。带 Studio Network Compilation 的 checkpoint 只保存 source roots，恢复时必须重新提供并验证原始 compilation。Loopback 的 URRF binding 也不是 WebSocket/UDP/QUIC/TLS 或真实第二网络证明。
