...
export class RealityNetworkRuntime {
  ...
  async joinCompiledSlotAuthority({sessionId,slotId,subjectId}={}){const ctx=this.require(sessionId),compilation=ctx.sourceCompilation;if(!compilation)throw new Error('SESSION_COMPILATION_REQUIRED');const slot=compilation.player_slots.find(item=>item.slot_id===slotId);if(!slot)throw new Error('COMPILED_SLOT_NOT_FOUND');const subject=subjectId??slot.subject_id;if(subject!==slot.subject_id)throw new Error('COMPILED_SLOT_SUBJECT_MISMATCH');const delegation=issuePlayerDelegation({sessionId,subjectId:subject,playerId:slot.player_id,characterId:slot.character_id,actions:slot.actions});const snapshot=ctx.server.joinPlayer({subjectId:subject,playerId:slot.player_id,characterId:slot.character_id,bodyId:slot.body_id,delegation});ctx.externalClients.set(slot.player_id,{playerId:slot.player_id,subjectId:subject,characterId:slot.character_id,bodyId:slot.body_id,connected:true,joinedTick:snapshot.tick});return{slotId,playerId:slot.player_id,delegation,snapshot};}
  ...
  reconnect({sessionId,playerId}={}){const ctx=this.require(sessionId);ctx.transport.reconnect(`client:${playerId}`);const client=ctx.clients.get(playerId),external=ctx.externalClients.get(playerId);if(client)client.connected=true;if(external)external.connected=true;const result=ctx.server.reconnect(playerId);if(client)client.reconcile(result.snapshot);return result;}
  getSessionHealth({sessionId}={}){const ctx=this.require(sessionId),server=ctx.server.health(),transport=ctx.transport.getStats(),clients={},externalClients={};for(const [id,client] of ctx.clients)clients[id]=client.metrics(server.stateRoot);for(const [id,client] of ctx.externalClients)externalClients[id]={...clone(client),serverStateRoot:server.stateRoot};const source=ctx.sourceCompilation?{format:ctx.sourceCompilation.format,compilationRoot:ctx.sourceCompilation.compilation_root,projectRoot:ctx.sourceCompilation.project_root,worldConfigRoot:ctx.sourceCompilation.world_config_root,spatialWorkspaceRoot:ctx.sourceCompilation.spatial_workspace_root,sourceWorldRoot:ctx.sourceCompilation.source_world_root,activeSceneRoot:ctx.sourceCompilation.active_scene_root,authoringRoot:ctx.sourceCompilation.authoring_root}:null;return {runtime_id:NETWORK_RUNTIME_ID,version:NETWORK_VERSION,protocol:NETWORK_PROTOCOL,server,transport,clients,externalClients,source};}
