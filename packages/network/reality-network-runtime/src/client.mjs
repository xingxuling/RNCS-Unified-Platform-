...
import {clone, FORMATS, makeCorrectionReceipt, NETWORK_PROTOCOL} from './protocol.mjs';
import {applyAuthoritativeStateDelta} from '@taowind/reality-simulation-runtime/network-reconciliation';
...
export class ClientPredictionRuntime {
  ...
  constructor({player,delegation,World}) {
    ...
    this.rollbackCount=0;this.replayCount=0;this.lastRejection=null;this.deltaBaseMismatchCount=0;this.lastDeltaMismatch=null;this.interpolator=new SnapshotInterpolator();this.connected=true;
  }
  ...
  createInput(command,{targetServerTick=this.currentSnapshot().tick+1}={}){
    ...
    const input={format:FORMATS.input,protocol:NETWORK_PROTOCOL,subjectId:this.player.subjectId,playerId:this.player.playerId,sessionId:this.player.sessionId,inputSequence:sequence,clientTick,targetServerTick,command:clone(command),authorizationRoot:this.delegation.delegation_root,clientPredictionRoot};
  ...
  receiveDelta(packet){
    ...
    let snapshot;
    if(packet.fullSnapshot&&packet.rsrSnapshot)snapshot=clone(packet.rsrSnapshot);
    else if(packet.rsrDelta){
      try{snapshot=applyAuthoritativeStateDelta(this.lastConfirmed.rsrSnapshot,packet.rsrDelta);}
      catch(error){if(error instanceof Error&&error.message==='RSR_DELTA_BASE_MISMATCH'){this.deltaBaseMismatchCount++;this.lastDeltaMismatch={expectedRoot:this.lastConfirmed.rsrSnapshot.stateRoot,deltaBaseRoot:packet.rsrDelta.baseStateRoot,tick:packet.tick};return null;}throw error;}
    }
    else {
      snapshot=clone(this.lastConfirmed.rsrSnapshot);const changed=new Map((packet.changedBodies??[]).map(body=>[body.id,body]));
      const removed=new Set(packet.removedBodyIds??[]);snapshot.tick=packet.tick;snapshot.bodies=snapshot.bodies.filter(body=>!removed.has(body.id)).map(body=>changed.has(body.id)?clone(changed.get(body.id)):body);
      for(const body of packet.changedBodies??[])if(!snapshot.bodies.some(existing=>existing.id===body.id))snapshot.bodies.push(clone(body));
      snapshot.bodies.sort((a,b)=>a.id.localeCompare(b.id));snapshot.characters=clone(packet.characters??snapshot.characters);snapshot.joints=clone(packet.joints??snapshot.joints);snapshot.listeners=clone(packet.listeners??snapshot.listeners);snapshot.contacts=clone(packet.contacts??[]);snapshot.events=clone(packet.worldEvents??[]);snapshot.diagnostics=clone(packet.diagnostics??snapshot.diagnostics);
      if(packet.roots)Object.assign(snapshot,clone(packet.roots));snapshot.stateRoot=packet.stateRoot;
    }
    const full={format:FORMATS.snapshot,protocol:NETWORK_PROTOCOL,sessionId:this.player.sessionId,reason:'delta-reconstruction',tick:packet.tick,worldId:snapshot.worldId,stepHz:snapshot.stepHz,objects:packet.objects??[],worldEvents:packet.worldEvents??[],stateRoot:packet.stateRoot,previousRoot:packet.previousRoot,serverReceiptRoot:packet.serverReceiptRoot,rsrAuthorityProtocol:packet.rsrAuthorityProtocol,rsrFrame:packet.rsrFrame,rsrSnapshot:snapshot};
  ...
  forcePredictionError(offset=1234){const snap=this.world.snapshot(),body=snap.bodies.find(b=>b.id===this.player.bodyId);if(!body)throw new Error('CLIENT_BODY_NOT_FOUND');const errorWorld=this.World.fromSnapshot(snap);errorWorld.step([{id:`client-error:${this.player.playerId}:${snap.tick+1}`,tick:snap.tick+1,type:'teleport',bodyId:this.player.bodyId,position:{...body.position,x:body.position.x+offset},rotationDeg:body.rotationDeg}]);this._world=errorWorld;this.cachedSnapshot=null;return this._world.snapshot().stateRoot;}
