...
func _process(_delta: float) -> void:
	if debug_label != null:
		debug_label.text = "Godot %s · FPS %d · LLM 0" % [Engine.get_version_info().get("string", "?"), Engine.get_frames_per_second()]
...
func _submit_incantation() -> void:
	if player == null:
		return
	var result := VoiceMagic.compile(voice_input.text)
...
func _build_ui() -> void:
	...
	quest_label.position = Vector2(930, 24)
	...
	announcement_label.position = Vector2(280, 105)
	...
	lock_label.position = Vector2(540, 610)
	lock_label.size = Vector2(260, 30)
	lock_label.horizontal_alignment = HORIZONTAL_ALIGNMENT_CENTER
	lock_label.text = "未锁定目标"
	root.add_child(lock_label)
	debug_label = Label.new()
	debug_label.position = Vector2(18, 580)
	debug_label.text = "Godot · FPS · LLM 0"
	root.add_child(debug_label)
	var help := Label.new()
	help.position = Vector2(18, 646)
	help.text = "WASD移动 · Ctrl冲刺 · 空格跳跃 · 鼠标视角 · 左键/J三段剑击 · Shift闪避 · Tab锁定 · R咏唱 · E交互 · 1-5法术 · F5保存"
	root.add_child(help)
	voice_panel = PanelContainer.new()
	voice_panel.position = Vector2(380, 220)
	...
	voice_status.text = "Web导出可调用浏览器语音识别；桌面版可输入咒语。"
