...
const outputDir = resolve(process.env.URRF_LARGE_WORLD_PBR_TEXTURE_RESIDENCY_OUT ?? join(rootDir, 'docs', 'verification', 'URRF_LARGE_WORLD_PBR_TEXTURE_RESIDENCY'));

test('streams and imports four-map PBR KTX2 residency for a large-world GLB candidate', async () => {
  mkdirSync(outputDir, {recursive: true});
  const runtime = new LargeWorldRuntime({
    worldId: 'world:urrf-large-world-pbr-texture-residency',
    seed: 'seed:urrf-large-world-pbr-texture-residency',
    width: 5,
    depth: 5,
    chunkSize: 64,
    sampleResolution: 8,
  ...
  const scene = runtime.createSpatialScene({
    selection,
    scene_id: 'urrf-large-world-pbr-texture-residency-v01',
    visual_scale: 1,
    evidence_root: rootHash({stream_root: stream.stream_root, selection_root: selection.selection_root, renderer: 'vsr-ragf-pbr-ktx2-residency'})
  });
  ...
  assert.equal(bundle.manifest.texture_profile, LARGE_WORLD_SPATIAL_GLB_KTX2_PBR_TEXTURE_PROFILE);
  ...
  const imported = await importGlbToSpatialSceneAsync(sourceEntry.payload, {
    sceneId: 'urrf-large-world-pbr-texture-residency-import-v01',
  ...
  assert.equal(imported.scene.textures?.length, 4);
  ...
  imported.scene.lights = [
    {id: 'light:pbr:ambient', kind: 'ambient', color: '#d9e7ff', intensity: .35},
    {id: 'light:pbr:sun', kind: 'directional', color: '#fff0ce', intensity: 1.2, direction: [-.45, -1, -.35], castShadow: false}
  ];
  ...
  const report = {
    format: 'urrf.large-world-pbr-texture-residency-report.v0.1',
    ...
    texture_count_per_asset: worldMetadata.texture_count,
    texture_roles: worldMetadata.texture_roles,
    texture_color_spaces: worldMetadata.texture_color_spaces,
    texture_level_count: worldMetadata.texture_level_count,
    texture_byte_length: worldMetadata.texture_byte_length,
    texture_residency_maps: worldMetadata.texture_residency.maps,
    ...
    imported_texture_count: imported.scene.textures.length,
    imported_texture_mip_count: imported.scene.textures[0].mipmaps?.length ?? 0,
    imported_material_texture_binding_count: imported.receipt.materialTextureBindingCount,
    ...
    notes: 'Candidate-only four-map native RGBA8 KTX2 PBR bundle embedded in binary GLB; normal, metallic-roughness, occlusion, emissive and base-color bindings, progressive mip planning, SHA-256 streaming, asynchronous VSR decode, frame compile, and CPU reference pixels execute locally. BasisU supercompression, target-device/GPU residency, CDN delivery, and production-scale performance remain unproven.'
  };
  report.report_root = rootHash(report);
  writeFileSync(join(outputDir, 'large-world-pbr-texture-residency-manifest.json'), `${JSON.stringify(bundle.manifest, null, 2)}\n`, 'utf8');
  writeFileSync(join(outputDir, 'large-world-pbr-texture-residency-report.json'), `${JSON.stringify(report, null, 2)}\n`, 'utf8');
  writeFileSync(join(outputDir, 'large-world-pbr-texture-residency-reference.png'), rendered.png);
  assert.match(report.report_root, /^[a-f0-9]{64}$/);
  assert.ok(readFileSync(join(outputDir, 'large-world-pbr-texture-residency-reference.png')).byteLength > 1000);
});
